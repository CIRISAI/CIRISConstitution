...
**Consent gates scoring, never accountability (normative).** A community, service, or node MAY condition admission, service, or continued participation on the subject holding a live `consent:scope:analyze` grant — consent-before-scoring is reciprocal, not an exemption. A subject that declines analysis cannot be scored; its `capacity:composite` is undefined and MUST NOT be emitted; and every gate that requires a capacity verdict therefore **fails closed** for that subject. Opting out of being scored is opting out of everything that requires a score — the choice is real in both directions, and neither direction is free.
