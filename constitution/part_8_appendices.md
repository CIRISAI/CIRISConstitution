...
| **R9** — Composite invertibility (aggregation ≠ erasure) | The [CC 6.1.2](part_6_the_coherence_mathematics.md) "already-erased by aggregation" shortcut rests on **procedure-relative** unrecoverability (a declared reconstruction procedure `R` above fidelity `ε`) plus purge-of-upper-tiers. **Bet**: for non-dominated composites this satisfies erasure obligations. **Mitigated since RC2**: *mass-based outlier dominance* is now normatively gated at admission — the signed `n_eff` dominance gate ([CC 6.1.2.1.2](part_6_the_coherence_mathematics.md), pinned `min_ratio = 0.5`, version-1 tiers fail closed), so a mass-dominated composite never enters a conformant store; where the gate fails, the N5 purge obligation applies in full. **Known exposure (residual)**: (1) the masses behind `n_eff` are *aggregator-attested* — the substrate cannot recompute them from the wire, so a lying `n_eff` is admitted (accountable-signature trust; a false `n_eff` is a slashable false attestation post-hoc, never wire-invisible); (2) *content-similarity multiplicity* — near-duplicate members under distinct ids at equal mass honestly pass; (3) *side-information / linkage reconstruction*. **Fallback (post-1.0)**: a formal DP-style bound on the aggregation operator (a content-similarity `N_eff` / max-multiplicity surface) — wire shape CIRISVerify#191, fold-time masses CIRISEdge#323, admission gate CIRISPersist#430, executable fixture CIRISConformance#71; definitional history in CIRISConstitution#6. |
