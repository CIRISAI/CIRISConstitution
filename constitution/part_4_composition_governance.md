...
**The Wise wear the same card (normative).** [WBD](part_1_foundation.md) routes judgment *to* a WA and the deferring agent then "awaits guidance; remains inactive on that issue" ([CC 1.9](part_1_foundation.md)) — so a WA seat is an authority-conferring relation, and an absent WA does not merely fail to answer, it freezes the agent. The seat therefore takes the [CC 4.5.1](#451-amendment--amendment-process--federation-contribution--wa-quorum--1-of-6-sign-off) swap test, and fails it unless the Wise are as legible as everyone the federation asks to be legible. Three symmetry rules, all riding shapes that already ship: **Denominator rule (normative):** any WA quorum threshold computes its denominator over the **full seated roster**, never the live subset — a shrinking live set MUST NOT lower a threshold, because an adversary able to silence seats would otherwise shrink the body to the seats it captured. Refusing more than required is the safe direction; refusing less is capture.

1. **Liveness is signalled, never assumed.** WA standing follows the [CC 4.5.13](#4513-reverse-quorum--reverse-quorum-governance--presence-is-authority-absence-forfeits-it) rule — *presence is authority, absence forfeits it*. A WA who does not answer within the quorum's stated freshness window lapses out of the live set `L` and is not in the denominator of a WA quorum ([CC 4.2.6](#426-live-quorum--live-quorum-operation--recovery-under-decimation-normative) participation shape, no new object). A deferral MUST NOT be routed to a WA outside `L`, and MUST NOT be left indefinitely open because its addressee is gone: on window expiry it re-routes to the live quorum. Return is Enoch-Arden — a WA who resurfaces rejoins going forward and does not reopen decisions taken by the then-live set.
