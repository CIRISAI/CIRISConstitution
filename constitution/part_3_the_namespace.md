...
The quantifier is **existential, and deliberately so**: node ownership is single-valued ([CC 3.2](part_3_the_namespace.md)) while agent stewardship is **multi-parent by design**, so the test is that the node's one owner appears *somewhere in* the agent's steward set — not that the agent has exactly one steward. **Fail-closed:** an unresolvable walk — owner absent, ambiguous owner, revoked edge — MUST refuse, and MUST NOT resolve to "unknown". This is the person/node axis, whose signature failure mode is a **silent withhold**; without the fail-closed half, an unresolved walk reads as permission.
...
**Open, and named rather than filled.** Clause D admits multi-tenant hosting only where the node's owner is *among* the hosted agent's stewards — which co-stewardship expresses directly, and which covers the community-server case. Hosting an agent whose steward set **excludes** the node owner is **not admissible** under this section; it would require an explicit conferral from the node owner, never a relaxation of Clause D. That is deferred, not denied.
