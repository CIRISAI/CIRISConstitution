...
**Status (the only rule a conformant implementation needs).** The binding `capacity:*` rule is the anti-self rejection above — `attesting_key_id MUST NOT equal attested_key_id` — and **nothing else**. No consent precondition gates `capacity:*` emission, and no read boundary redacts `capacity:*` from any party the [CC 5.3.2.4.4](part_5_transport_substrate.md) scope gate admits. Both surfaces are **reserved design space**: a consent-before-scoring gate and an anti-Goodhart read boundary are under design at CIRISConstitution#46 / #49, and the adversarial analysis any future ratification must answer — capture via owner-sibling keys, third-party-dissent removal, the exemption boundary for the duty-holder families, and the scope-not-dimension read model — is recorded there, not here. A deployment MUST NOT enforce either surface ahead of ratification and call itself conformant.
