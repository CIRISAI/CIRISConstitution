...
The federation is symmetric by design — every participant binds every other under the Recursive Golden Rule. The **wire format** carries exactly one asymmetry, and it points outward: humanity's right to halt the system. That claim is about the wire only: governance carries further asymmetries — bootstrap ones, listed on the [CC 4.5.1](#451-amendment--amendment-process--federation-contribution--wa-quorum--1-of-6-sign-off) declared-asymmetry register — and the register is not yet exhaustive. This section specifies it. The asymmetry is **consent-scoped**: it reaches exactly those holding a live trust edge to the accord, and it dies when that edge is cut ([CC 4.2.1](#421-authority--authority-scope) reach, [CC 4.4.3.8](#4438-policy-direct--policy-a--direct-trust) un-trust). The humanity accord is *a* trust root, not *the* trust root; what it holds over those who root to it is an emergency brake they granted, not a power imposed on everyone.
...
**Root conferral is in scope.** The enumeration admits a 2-of-3 accord co-scrub over a `canonical,node` registration envelope bearing `roles: ["infra:serve"]` — the ceremony that mints a **portable trust root** ([CC 4.4.3.8](#4438-policy-direct--policy-a--direct-trust)). Read without it, the clause forbids the accord from blessing the very primitive that creates a root. Scope isolation still does its real work: accord authority cannot reach the consent or licensure planes. It does **not** isolate the amendment plane — accord-holders are named in [CC 4.5.1](#451-amendment--amendment-process--federation-contribution--wa-quorum--1-of-6-sign-off) step 5 (1-of-6 sign-off, with veto), in the pre-maturity authority of that section, and in the [CC 4.5.1.2](#4512-meta-amendment--meta-amendment--entrenchment) entrenched 2-of-3 ratification. That reach is declared and bounded on the CC 4.5.1 register; it is not denied here. Root-minting is **not** an accord privilege — the identical ceremony under any other root's holders mints an equally valid root, and the CIRIS root is the shipped default, not a privileged one ([CC 3.2](part_3_the_namespace.md): a default-plus-re-root is a federation). Conferral MUST NOT be relocated to a founder-quorum of an infrastructure community: that is circular at genesis — a community presupposes admission, which presupposes a root — and destroys portability.
...
**A consensual authority is a different object from an imposed one.** The reach rule at [CC 4.2.1](#421-authority--authority-scope) is what makes this a *safe-mesh* kill switch rather than a backdoor: the party bound is the party who granted, and can ungrant. That is also how the clause survives the [CC 4.5.1](#451-amendment--amendment-process--federation-contribution--wa-quorum--1-of-6-sign-off) swap test — swap the accord holders for the least powerful participant and re-read: each still binds only those who chose them, each still cannot escape a halt already fired, each can be un-trusted with the base self-root intact. What does *not* survive the swap is genesis: the trio holds at day zero what nobody else can yet assemble. That residual is carried as a **declared asymmetry** on the [CC 4.5.1](#451-amendment--amendment-process--federation-contribution--wa-quorum--1-of-6-sign-off) register — listed, bounded, revocable, and scope-isolated — never as a silent exemption.
...
**The swap test — a mandatory drafting gate (normative).** Every clause that **confers authority or gates a capability** MUST pass the swap test before adoption: *swap the most and least powerful parties the clause names or implies, re-read it, and ask whether it still reads acceptable; if it does not, it was a privilege, not a rule.* This is a drafting gate on what CC may say, never an adjudication rule — it binds amendment and the founder's pre-maturity authority alike. Where the test **fails**, the outcome is recorded on the declared-asymmetry register below. There is at present **no recording surface for a passing test** — the registry carries no field for one and no clause records one — so absence from the register does **not** yet assert that a clause passed; it asserts only that no one has listed it. Making silence mean something requires a per-clause record that does not exist, and the register's own incompleteness note below stands until it does.

**Declared asymmetry — the form a necessary asymmetry must take (normative).** Some clauses fail the swap test and are still necessary; bootstrap is the standing case, because genesis grants a named few what nobody else can yet assemble. CC does not resolve this with silent exemptions. Such a clause is either rewritten until it passes, or carried as a **declared asymmetry**, which MUST satisfy all four: **listed** (an unlisted asymmetry is a defect, not an exception); **bounded** (carrying the condition that ends it, not a discretionary review date); **revocable** (an exit the disadvantaged party can take unilaterally); and **scope-isolated** (the authority cannot reach beyond its enumerated surface, so compromise cannot escalate).

| Declared asymmetry | Bounded by | Exit | Isolation |
|---|---|---|---|
| **Founder / accord-holder amendment authority** — the maturity gate above | the mesh reaching maturity (working threshold ≥100,000 nodes), at which point the mechanized process supersedes it and governance tightens, never loosens | fork under the licence; no one is bound who does not voluntarily join the instance | the rule layer only — it confers no halt, no grant, no licensure |
| **Genesis root-minting** — the accord triple can confer the first `canonical` when no prior root exists ([CC 4.2.1](#421-authority--authority-scope)) | the existence of a second root: anyone may mint one by the same ceremony, and the privilege is first-mover, not structural | un-trust and re-root ([CC 4.4.3.8](#4438-policy-direct--policy-a--direct-trust)); the base self-root keeps the installation running meanwhile | conferral only — accord authority cannot reach the consent or licensure planes. It is **not** isolated from the amendment plane; that reach is entry 1 above |

The [CC 4.2](#42-accord--the-humanity_accord-constitutional-layer) halt itself is **not** on this register: it passes the swap test on the [CC 4.2.5](#425-isn--why-this-isnt-a-golden-rule-violation) reading, because the party bound is the party who granted and can ungrant.

**The register is not exhaustive — stated, not hidden.** Two further asymmetries are known to be unlisted, and neither is resolved into a row here because neither has a bound and an exit that could be written down without inventing one. First, the **H7 steward restore-to-known-good** power ([CC 4.2.6](#426-live-quorum--live-quorum-operation--recovery-under-decimation-normative)) — described in that section as a sanctioned exception to entrenchment — confers on the regional stewards authority to mutate the accord roster, with **no terminating condition** and **no unilateral exit** for a party subject to it. Second, the **1-of-6 veto** at step 5 above is not ended by entry 1's maturity bound: at maturity it changes shape from *decides* to *blocks* and persists. Until each is either rewritten until it passes, or listed with a real bound and a real exit, this register states a form rather than a complete accounting, and no reader should treat absence from it as a pass. **Revision condition:** if the register grows past a handful of entries, or if an entry's stated bound passes without the asymmetry ending, the form has failed as a discipline and the surviving asymmetries should be re-read as permanent structure requiring [CC 4.5.1.2](#4512-meta-amendment--meta-amendment--entrenchment) entrenched ratification rather than declaration.
...
- **Capacity self-emission ([CC 3.4.5](part_3_the_namespace.md))**: **unaffected by role cohabitation** — which is the only claim made here. CC 3.4.5 itself was substantially revised in this candidate (a consent-before-scoring gate and a read boundary were added there and are being carried separately); nothing in this section ratifies, defers, or restates that material. What is unchanged is the interaction: the anti-Goodhart `attesting_key_id ≠ attested_key_id` rule binds regardless of how many roles a key holds. A folded `{agent, lenscore_detector}` key still MUST NOT score its own `capacity:*`. Role cohabitation does not create a self-attestation backdoor.
