...
| `trust:*` ([CC 3.1.1](part_3_the_namespace.md)) | **Edge-source-only, per job.** These ride `delegates_to` rows, not `scores`, and are enforced under the same discipline by the [CC 3.4.7](part_3_the_namespace.md) scope sentence. `trust:charter:v1` MUST be self-referential — `attesting_key_id` = the delegating key = the delegated key — so a charter is only ever a key's own declaration about itself. `trust:confers:v1` MUST be emitted by the conferring root, which MUST be the source of the edge. `trust:accepts:v1` MUST be emitted by the **accepting node**, which MUST be the source of the edge; a root MUST NOT emit an acceptance edge naming itself as the accepted root. A row failing its job's rule is rejected at admission, not scored down. | Without this rule nothing binds `trust:*`, and any key may mint a charter *for another key* or manufacture its own acceptance — which would defeat the [CC 3.2](part_3_the_namespace.md) T3 one-row un-trust lever, since the deletion the node makes would be re-created by the root. Rootness is a claim a key makes about itself and a node accepts about the root; neither half may be authored by the other party. |
