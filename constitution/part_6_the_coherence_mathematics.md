...
under the stated preconditions — the cut region is convex, its radius `r < 0.5` (the theorem's decay rate `λ_geo ≈ 2r`), constraint orientations are Grassmannian-uniform, and `k ≥ 3`. This geometry grounds the [CC 3.1.8.2](part_3_the_namespace.md) Coherence-Ratchet detectors and the [CC 3.1.8.4](part_3_the_namespace.md) F-3 correlated-action detector, which read the same `ρ` / `k_eff` structure over federation-emitted traces.
...
**J is monotone in diversity — the corridor is a claim about a different quantity (normative citation discipline).** `J` is strictly decreasing in `ρ̄` and is maximised at `ρ̄ = 0`: it is a **throughput index**, and nothing in this section supports an intermediate "healthy band" of correlation. `J` MUST NOT be cited as the basis of the operating corridor. The corridor — that a federation must sit strictly between total rigidity and total fragmentation — is a **capacity** statement about the **whole-only share**: the pattern carried by a collective and absent from every part of it. Its basis, at these strengths and no stronger:

- **Both poles are exactly zero — theorem-given-model, machine-checked.** A flip-symmetric (perfectly rigid) structure carries zero whole-only pattern (`lean:CIRISOntology:Core.share_eq_zero_of_signSymmetric`); fully independent parts carry zero (`lean:CIRISOntology:Core.Valve` product lemmas — independence carries zero). Any positive share therefore requires being at neither pole, so the maximum lies **strictly between** them. Scope: three slots (`k = 3`); sign symmetry kills only the odd orders, and symmetric noise mints even-order structure from four slots up, so a monitor reading more than triples must budget that floor.
- **The between-maximum is exact — theorem-given-model.** The per-triple ceiling is exactly `ln 2` (`lean:CIRISOntology:Core.ThirdCap.share_max_eq_log_two`), attained by parity and crossed by nothing, with **no hypothesis on the marginals**.
- **Where the peak sits is measured, not proved — measured.** In model systems under weak symmetry breaking the quantity peaks at criticality in two universality classes, confirmed by a forward prediction staked in advance (`−3.084 ± 0.219` observed vs `−3.109` predicted); observed ceiling fractions are sub-percent. That the peak sits at criticality **in this federation** is *not* established.
- **The corridor is maintained, not attractive — theorem-given-model.** Both poles are attractors under drift and the corridor is not: it persists only under non-trivial maintenance (`lean:coherence-ratchet:Core.Dynamics.corridor_requires_maintenance`, `Core.Dynamics.rho_exit_chaos`). The operating band pinned for `ρ̄` is `(ρ_lower, ρ_upper) = (0.1, 0.43)`, well-formed as an open interval (`lean:coherence-ratchet:Core.Corridor.corridor_bounds_well_formed`), giving asymptotic `k_eff ∈ (1/ρ_upper, 1/ρ_lower) ≈ (2.33, 10)` (`Core.Corridor.corridor_keff_range_asymptotic`). Honest limit: those bounds rest on the `corridorBounds` axiom — the **numbers are posited, not derived**; the theorems say the interval is well-formed and what `k_eff` range it implies, not that `0.1` and `0.43` are the right endpoints. Direct-read `k_eff` across six substrates lands at `≈ 7 ± 2` with a ceiling near 10 (**measured**), consistent with `1/ρ_lower`.

The two quantities are not interchangeable: `k_eff` is a Kish effective count over constraint orientations, the whole-only share is a capacity of a collective over its parts, and a claim about one MUST NOT be transferred to the other.
...
**Nomenclature note (C vs F).** This composite is written **F**. The symbol **C** is reserved for the **core-identity factor** of the per-agent CIRIS Capacity Score `𝒞_CIRIS = C · I_int · R · I_inc · S` ([CC 3.1.8.1](part_3_the_namespace.md)). **F** (this three-factor, federation-level flourishing capacity) and **𝒞_CIRIS** (the five-factor per-agent score) are **distinct composites with no implied mapping** between them. This section is the statement of record for that relation — no external work is authority for it — and [CC 3.1.8.1](part_3_the_namespace.md) carries the matching note at the point where `𝒞_CIRIS` is introduced.
