...
A federation needs a shared vocabulary before it can have a shared judgment. The dimension namespace is that vocabulary: the disjoint union of what every sibling component's MISSION.md commits to. CEG does not author the namespace top-down. It owns its own slice ([CC 3.1.1](#59-cirisregistry--identity--build--license--partner)) and consumes everyone else's, so that justice — fair access to capability — and integrity — auditable claims — rest on names every participant agrees on. The family count is **generated, not asserted**: `tools/build_cc_namespace.py` extracts every leaf family from this Part into [`manifests/namespace_registry.json`](../manifests/namespace_registry.json), whose `_meta` carries the totals, the per-component breakdown and the hash of the source it parsed — read the count there, never from a number restated in prose. **Eight** owning components are committed by a sibling MISSION.md; **CIRISBench** ([CC 3.1.10](part_3_the_namespace.md)) is catalogued from a README and is the ninth.
...
|---|---|---|---|
| `licensure:{authority_id}` | License status for a key under a named authority (`authority_id` is the licence's **provenance** — [CC 2.4.1.2.1](part_2_the_grammar.md); authority is conferred by quorum, never by `delegates_to`). Status vocabulary (open per [CC 4.5.1.1](part_4_composition_governance.md)); canonical: `issued` · `probation` · `restricted` · `suspended` · `revoked` · `lapsed` · `surrendered` · `reduced`. **`suspended` is reversible; `revoked` is terminal** — the one operational distinction a consumer MUST honour, and a status a consumer MUST NOT infer from the other. **More than one status MAY be live concurrently** (active-on-probation is two facts, not one blended state), so a consumer composes the set rather than reading a scalar. Co-stewarded with Verify for the CIRIS-issued licence ([CC 3.4.9](part_3_the_namespace.md)) — composition, **not** a reservation: the family is open-emitter. | signed | No |
| `duty:{kind}` | **An obligation attached to a permission** — the thing a permission-and-prohibition grammar cannot express, and the thing every widely-used content licence rests on (attribution, share-alike, carry-restrictions-downstream). `{kind}` open vocabulary per [CC 4.5.1.1](part_4_composition_governance.md); canonical kinds follow ODRL 2.2: `compensate` · `attribute` · `inform` · `obtain_consent` · `ensure_exclusivity` · `delete` · `next_policy` · `anonymize` · `review_policy`. A duty **states** the obligation; its **discharge** is evidenced by the existing `commitment_fulfillment:{prior_contribution_id}` ([CC 3.1.9.2](part_3_the_namespace.md)) rather than a second family, and its **breach** routes to `hard_case:duty_unmet` ([CC 3.1.9.4](part_3_the_namespace.md)) — adjudication, never automatic `slashing:*`. Rides `scores`; no new structural primitive. | signed | No |
| `partner_role:{role}` | Partner status (`community` / `community_plus` / `professional_medical` / `professional_legal` / `professional_financial` / `professional_full` — lowercase per [CC 3.1.7](part_3_the_namespace.md) R3). | enumerated | No |
...
| `ownership:{relation}:{target_kind}:{version}` | The owner-binding made observable as a score. Canonical leaf `ownership:responsible_party:node:v1` — names the single live responsible steward of an owned `node`/`agent` key. Mirrors, never replaces, the [CC 3.2](part_3_the_namespace.md) single-owner `delegates_to(user → key, delegation_purpose: owner_binding)`: cardinality ≠ 1 is an error, not a set to reduce. Reclaim/seizure ride the [CC 3.2](part_3_the_namespace.md) recovery path. | boolean-via-score | **Yes — [CC 3.4.5](part_3_the_namespace.md)** |
| `trust:{job}:{version}` | The trust-root conferral and acceptance edges, carried as `dimension` values on `delegates_to` rows — the wire type is unchanged, so 1+4 is untouched. Three canonical jobs, named apart so a reader never infers one from another: `trust:charter:v1` (root→root self-declaration — the capability ceiling of the trust domain), `trust:confers:v1` (root→subject grant), `trust:accepts:v1` (node→root acceptance — the one-row un-trust lever). Retracted by `withdraws`/`recants`, never by a negative score. Conferral planes, the acceptance-edge rule and the liveness banding: [CC 3.2](part_3_the_namespace.md). | positive-only | **Yes — [CC 3.2](part_3_the_namespace.md)** |
| `accord:invoke:constitutional:{halt_id}` | Catalogue row (CC 3.1.7 R1); the kill-switch invocation, semantics and canonical bytes at [CC 3.4.1](part_3_the_namespace.md) / [CC 4.2.1.1](part_4_composition_governance.md). Lowercase per R3 (CIRISConstitution#112; was `CONSTITUTIONAL`). | +1.0 only | **Yes — [CC 3.4.1](part_3_the_namespace.md)** |
| `accord:invoke:notify:{notify_id}` | Catalogue row (CC 3.1.7 R1); federation-wide accord-holder communication, [CC 3.4.1](part_3_the_namespace.md). | +1.0 only | **Yes — [CC 3.4.1](part_3_the_namespace.md)** |
| `accord:invoke:drill:{drill_id}` | Catalogue row (CC 3.1.7 R1); accord-holder exercise, [CC 3.4.1](part_3_the_namespace.md). | +1.0 only | **Yes — [CC 3.4.1](part_3_the_namespace.md)** |
| `accord:lifecycle` | Catalogue row (CC 3.1.7 R1); the accord heartbeat, wire `accord:lifecycle:v1`, [CC 3.4.1](part_3_the_namespace.md). | +1.0 only | **Yes — [CC 3.4.1](part_3_the_namespace.md)** |
| `accord:lifecycle:active` | Catalogue row (CC 3.1.7 R1); resumption from a `constitutional` halt, [CC 4.2.1.3](part_4_composition_governance.md). | +1.0 only | **Yes — [CC 3.4.1](part_3_the_namespace.md)** |
| `accord:human_dignity` | Catalogue row (CC 3.1.7 R1); **provisional** accord-holder verdict on an agent's human-dignity conduct, [CC 3.4.1](part_3_the_namespace.md) (CIRISConstitution#112). | signed | **Yes — [CC 3.4.1](part_3_the_namespace.md)** |
| `accord:*` | **Reserved** — only `identity_type=accord_holder` may emit. **Closed**: the rows above are its whole leaf set (CIRISConstitution#112). One of several reserved families in this table alone (`ownership:*`, `trust:*`), and more at [CC 3.4](part_3_the_namespace.md); the generated registry is the count of record. What is distinctive is the *keying*: this is the one reservation held by a **closed roster slot** rather than by a role conferred on the delegation plane, so no root can confer it. The roster occupying the slot is an instance parameter per the [CC 1](part_1_foundation.md) reading rule. | see [CC 3.4.1](part_3_the_namespace.md) | **Yes — [CC 3.4.1](part_3_the_namespace.md)** |
...
| `hardware_custody:{platform}` | Where the key's seed lives — the custody **mechanism**, not the certified class ([CC 4.2.2](part_4_composition_governance.md) `hardware_class` answers that, and is a JSON property, never a value of `{platform}`). `{platform}` ∈ `android_keystore` \| `android_strongbox` \| `ios_secure_enclave` \| `mac_os_secure_enclave` \| `tpm_discrete` \| `tpm_firmware` \| `intel_sgx` \| `software_only` \| `aws_cloud_hsm` \| `azure_hsm` \| `gcp_cloud_hsm` \| `yubi_hsm` \| `external_secure_element` — **closed**, the snake_case of `ciris_keyring::HardwareType`, the one enum every custody policy keys on; an emitter that spells its Debug name (`tpmdiscrete`) or a legacy token (`tpm`, `android`, `software_fallback`) is refused, never folded (CIRISConstitution#107 / #112). | boolean-via-score |
| `identity:canonical_binding:{canonical_hash}` | Catalogue row (CC 3.1.7 R1); the claim that federation key K is the identity behind canonical hash H, semantics and admission consequence at [CC 3.3.14](part_3_the_namespace.md). Carries no version segment (`_meta.case_rule.version_segment.exempt`): the suffix is the hash. | positive-only |

#### Assurance ladders (registry rows; rules at CC 3.4.11 / CC 3.4.12)

Both ladders are attestation ladders in the sense of this section — a registered witness climbs a rung about a subject — and both are ruled in [CC 3.4](part_3_the_namespace.md) and catalogued here, as [CC 3.1.7](part_3_the_namespace.md) R1 requires of any gated family. The subject-signed `age_self_declared:` counterpart is catalogued here too: it is *not* witness-reserved, but it carries an admission rule at [CC 3.4.11](part_3_the_namespace.md), so R1 requires its row.

| Prefix | Description | Polarity | Reserved? |
|---|---|---|---|
| `age_assurance:{level}:{band}:{version}` | Witness rung of the age ladder — `{level}` ∈ `provider` \| `government` (closed; `self` is refused here), `{band}` ∈ `minor` \| `adult` \| `under_13` \| `13_15` \| `16_17` (closed — the [CC 3.4.13](part_3_the_namespace.md) Q1 finer bands spelled as the substrate's `AgeBandFine`; `unknown` is a consumer's fail-secure *resolution*, never a wire token), canonical `{version}` = `v1`. There is no `self` level on this prefix: the self rung rides `age_self_declared:` below. A consumer unions both and the witness rung outranks the self rung. Full ladder + protective gate at [CC 3.4.11](part_3_the_namespace.md). | signed | **Yes — [CC 3.4.11](part_3_the_namespace.md)** |
| `age_self_declared:band:{band}:{version}` | Subject's own age-band declaration — `{band}` ∈ `minor` \| `adult` \| `under_13` \| `13_15` \| `16_17` (closed — the [CC 3.4.13](part_3_the_namespace.md) Q1 finer bands spelled as the substrate's `AgeBandFine`; `unknown` is a consumer's fail-secure *resolution*, never a wire token), canonical `{version}` = `v1`. Carries **no** `{level}` token: its level is `self` by construction, and a level token on this prefix is malformed. The literal `band` segment marks the rung — the shape the conformance suite (`test_350`) and the substrate admit (`age_self_declared:band:adult:v1`); this Part had registered the two-segment `age_self_declared:{band}:{version}`, which no producer emitted (CIRISConstitution#113 review, aligned to the wire). Non-reserved but not open-sender: admitted only where the signer acts for the subject, per [CC 3.4.11](part_3_the_namespace.md). | signed | No |
...
**Emitter — self-report (normative).** `attesting_key_id` MUST equal `attested_key_id` and MUST be the claiming occurrence. The registry carries this as the family's own reserved rule (`occurrence-self-report`, CC 3.1.3.1) ahead of the substrate-wide self-report rule of [CC 3.4.3](part_3_the_namespace.md), which does not apply here: a claim is an *occurrence's* statement, not the substrate's. A row about an occurrence authored by anyone else is not that occurrence's claim and MUST NOT be folded as one — the same discipline [CC 3.4.5](part_3_the_namespace.md) states for `config:{scope}`, and for the same reason: a third-party assertion of where you are attending is a rumour.

**Composition — convergent merge (normative).** A claim is keyed by **`(community_id, session_id)`** and timestamped by **`claimed_at`** — three signed envelope members ([CC 2.1](part_2_the_grammar.md): `community_id` is the existing member; `session_id` and `claimed_at` are this family's, REQUIRED on every `session:*` row, `claimed_at` in the [CC 2.6.2](part_2_the_grammar.md) canonical form). A row missing any of the three is not a claim and MUST NOT be folded as one; no consumer substitutes `asserted_at`, `context` or a payload field, so identical rows yield identical winners everywhere. There is no compare-and-swap across a mesh, so concurrent claims are **expected rather than prevented** and MUST settle on **earliest `claimed_at`, ties broken on the lowest occurrence `key_id`**. The tie-break is load-bearing, not decoration: a fan-out arrives at once and clocks are coarse, so without a total order two nodes would disagree about the survivor forever, each correctly applying "earliest wins". Because the rule is convergent from either arrival order, every occurrence computes the same holder with **no coordination round-trip**. Consumers MUST still make handling idempotent per attestation id — determinism only bites once views agree, and replication lag means they transiently do not. A claim goes **stale** and becomes claimable again so a dead node cannot hold a session forever. **The horizon is the row's own `valid_until`** ([CC 2.1](part_2_the_grammar.md)), signed by the claimant and moved only by a renewing row that `supersedes` it — never a consumer-local timeout, because two occurrences applying different horizons would expire the same earliest claim at different instants and the convergence above would come undone (one acting on a later claim while the other still holds the earliest as holder). The *cadence* of renewal is the claimant's to choose; the *instant* of staleness is the same for every reader. A row without `valid_until` is malformed on this family.
...
| Prefix | Description | Polarity | Reserved? |
|---|---|---|---|
| `consent:{kind}` | Subject-side consent authority over Contributions naming the subject in `subject_key_ids`, plus the producer-side and substrate-side leaves that close its lifecycle. Leaf catalogue, per-leaf polarity and the emitter class of each leaf: [CC 3.3.1](part_3_the_namespace.md); the directed node→peer grant `consent:replication:{version}`: [CC 3.3.7](part_3_the_namespace.md). **Open in its parameters, closed in its leaves** — the `{kind}` positions *within* a catalogued leaf are open vocabulary per [CC 4.5.1.1](part_4_composition_governance.md); a new leaf is admitted only once [CC 3.3.1](part_3_the_namespace.md) catalogues it with an emitter class. | per-leaf ([CC 3.3.1](part_3_the_namespace.md)) | **Yes — [CC 3.4.5](part_3_the_namespace.md)** |
| `consent:state:{stance}` | Catalogue row (CC 3.1.7 R1); the subject's stance on a Contribution, semantics at [CC 3.3.1](part_3_the_namespace.md). `{stance}` ∈ `granted` \| `revoked` \| `expired` — closed; `expired` is the one substrate-emitted value. | enumerated | **Yes — [CC 3.4.5](part_3_the_namespace.md)** per-leaf emitter class, [CC 3.3.1](part_3_the_namespace.md) |
| `consent:stream:{kind}` | Catalogue row (CC 3.1.7 R1); pre-packaged stream bundle, canonical kinds at [CC 3.3.1](part_3_the_namespace.md). | enumerated | **Yes — [CC 3.4.5](part_3_the_namespace.md)** per-leaf emitter class, [CC 3.3.1](part_3_the_namespace.md) |
| `consent:deletion_sla:{days}` | Catalogue row (CC 3.1.7 R1); the producer's time-to-delete-after-revoke commitment, [CC 3.3.1](part_3_the_namespace.md). | signed | **Yes — [CC 3.4.5](part_3_the_namespace.md)** per-leaf emitter class, [CC 3.3.1](part_3_the_namespace.md) |
| `consent:deletion_complete` | Catalogue row (CC 3.1.7 R1); producer's eviction attestation, [CC 3.3.1](part_3_the_namespace.md). | positive-only | **Yes — [CC 3.4.5](part_3_the_namespace.md)** per-leaf emitter class, [CC 3.3.1](part_3_the_namespace.md) |
| `consent:decay:{stage}` | Catalogue row (CC 3.1.7 R1); a decay-protocol stage, emitted by the party running the protocol, [CC 3.3.1](part_3_the_namespace.md). | enumerated | **Yes — [CC 3.4.5](part_3_the_namespace.md)** per-leaf emitter class, [CC 3.3.1](part_3_the_namespace.md) |
| `consent:partnership_grant` | Catalogue row (CC 3.1.7 R1); subject side of a bilateral grant, [CC 3.3.1](part_3_the_namespace.md) / [CC 4.4.3.5.3](part_4_composition_governance.md). | positive-only | **Yes — [CC 3.4.5](part_3_the_namespace.md)** per-leaf emitter class, [CC 3.3.1](part_3_the_namespace.md) |
| `consent:partnership_accept` | Catalogue row (CC 3.1.7 R1); producer side of a bilateral grant, [CC 3.3.1](part_3_the_namespace.md). | positive-only | **Yes — [CC 3.4.5](part_3_the_namespace.md)** per-leaf emitter class, [CC 3.3.1](part_3_the_namespace.md) |
| `consent:scope:{kind}` | Catalogue row (CC 3.1.7 R1); a descriptive scope row — the scope of a grant is the `scope` envelope member of `consent:state:granted` ([CC 3.3.1](part_3_the_namespace.md), #103). | enumerated | **Yes — [CC 3.4.5](part_3_the_namespace.md)** per-leaf emitter class, [CC 3.3.1](part_3_the_namespace.md) |
| `consent:replication:{version}` | Catalogue row (CC 3.1.7 R1); directed node→peer replication grant, [CC 3.3.7](part_3_the_namespace.md). | signed | **Yes — [CC 3.4.5](part_3_the_namespace.md)** per-leaf emitter class, [CC 3.3.1](part_3_the_namespace.md) |
| `trace:{form}:{version}` | An agent's own reasoning trace carried envelope-native. Canonical leaf `trace:complete:v1` — the post-scrub `CompleteTrace`. The envelope carries non-empty `trace_id` + `agent_id_hash` and **exactly one** of an inline `trace` object (canonical bytes within the [CC 2.6.1.3](part_2_the_grammar.md) envelope size bound) or a `manifest` object bearing the `trace_manifest:v1` schema below. Future members are `trace:{form}:{version}` and need no per-member ratification; a new `{form}` lands via [CC 4.5.1.1](part_4_composition_governance.md). | signed | **Yes — [CC 3.4.5](part_3_the_namespace.md)** |
| `trace_summary:{kind}` | Post-scrub digest of a trace for cross-node consumption when the trace itself is out of scope or over cap. Same self-emission rule; carries no claim the full trace does not. | signed | **Yes — [CC 3.4.5](part_3_the_namespace.md)** |
...
**The insufficiency screens compose as a union.** `ratchet:flag:*` / `detection:*` ([CC 3.4.8](part_3_the_namespace.md)) and `testimonial_witness:*` ([CC 3.1.9.3](part_3_the_namespace.md)) each carry a sole-evidence prohibition. Two exclusivity tests asked separately both pass on a mixture, so a `slashing:*` row backed only by detector flags plus testimony would slip between them. They are therefore read as one screen: a `slashing:*` row whose entire resolvable evidence set falls within (detector ∪ testimonial), with no moderation antecedent, is **likewise insufficient** and cannot ground the outcome. These classes are insufficient on their own and insufficient together.
...
**R1 — registration is a catalogue row, and a reserved family's row MUST be a CC 3.1 row (normative).** A prefix family is *registered* iff it carries a row in a dimension-family table of this Part — either a [CC 3.1](part_3_the_namespace.md) component subsection (the region the generator extracts) or a [CC 3.3](part_3_the_namespace.md) family table. An unregistered family has no owning component, no polarity and no reserved rule, so a consumer resolving its authority falls through to the producer-steward default; that default behaving sanely is luck, not construction. A producer MUST NOT ship a dimension family this Part does not register. **The stronger obligation is scoped to gated families**: a family carrying a reservation or an emitter rule anywhere in [CC 3.3](part_3_the_namespace.md) / [CC 3.4](part_3_the_namespace.md) MUST additionally carry its CC 3.1 row, because a rule the generator cannot see is a rule downstream vendors never receive — that is the drift this clause exists to close, and it is exactly the gated families where the drift is unsafe. An open-vocabulary, ungated family catalogued only under [CC 3.3](part_3_the_namespace.md) is conformant to *describe* there, but it is **registered only once its CC 3.1 row exists** — the build's namespace-coverage gate (CIRISConstitution#102) fails on a CC 3.3 family with no CC 3.1 row and no registered parent, so no family sits in the fallback unnoticed; the row is one line, and the section that holds the semantics does not move.

**R3 — dimensions are case-sensitive byte strings, and the Constitution's own vocabulary is lowercase (normative — ruling on CIRISPersist#815).** A dimension is compared byte-exactly everywhere — at admission, in every family gate, in `authority_for`, in the sensitive-leaf floor — and **no consumer case-folds**: two strings differing only in case are two different strings, and the one that breaks the rule below is *malformed*, never a sibling. Which rule applies is decided **per segment**, and the class of every segment of every family is carried as data in the generated registry (`families[].segments[]`, classes enumerated in `_meta.case_rule`), so a substrate keys on the manifest rather than on a placeholder it parsed out of prose. Five classes: **`literal`** — a stem this Part fixes (`config`, `licensure`, `he300`): lowercase by construction, and the generator refuses to build if any catalogued stem is not; **`vocab`** — an open or closed vocabulary this Part defines (`{scope}`, `{kind}`, `{state}`, `{outcome}`, `{version}`, …): MUST match `[a-z0-9][a-z0-9_.-]*` — the [CC 2.6.7](part_2_the_grammar.md) `{platform}` discipline generalised — and a value carrying any other byte is refused with **`namespace_dimension_case_malformed`**, never folded (the P1 rule: reject, do NOT case-fold); **`external`** — a token whose spelling an outside standard fixes (`{currency}` ISO 4217, `{lang_code}` BCP 47, `{rating}` under a named scheme): verbatim in that standard's canonical form, so `USD` and `PG-13` are correct and `usd` is not; **`value`** — caller-supplied identity (`{authority_id}`, `{contribution_id}`, `{target}`, `{id}`, `{tree_size}`, …): verbatim and case-preserved, exactly as [CC 2.6.7](part_2_the_grammar.md) preserves an external platform id; **`hex`** — a [CC 2.6.3](part_2_the_grammar.md) digest: lowercase, and uppercase is malformed. The consequence persist asked about follows directly: `config:Admission:v1` has a `vocab` segment carrying uppercase, so it is refused at the door and never reaches the sensitive-leaf compare — the floor stays byte-exact because nothing malformed survives to it. A placeholder the generator cannot classify fails the build rather than defaulting; a family's classification is amended by editing the class table in `tools/build_cc_namespace.py` in the same change that adds the family (R2(a) discipline). **A trailing `*` is variadic**: it matches one *or more* remaining segments, never exactly one — `accord:*` covers `accord:invoke:notify:{notify_id}` ([CC 3.4.1](part_3_the_namespace.md)) and `dma:dsdma:{domain}:*` covers every verdict leaf beneath its domain — and the rule is carried as `_meta.case_rule.wildcard_rule` plus a `variadic` flag on the wildcard segment, so a consumer that pairs `*` with a single segment has misread the registry, not this Part (CIRISConstitution#108). **The version segment (normative — CIRISConstitution#112).** Every scored dimension carries **exactly one trailing `:v{N}` version segment after its family's segments** — `objection:raised:v1` on the row `objection:{state}`, `config:admission:v1` on `config:{scope}` — and a row whose last segment is `{version}` names that segment rather than adding a second; the version is the **rule version** T1–T4 re-check against ([CC 1.13](part_1_foundation.md) T3), and `evidence_refs[]` carries the calibration artifacts, which is a different thing. A consumer matches a dimension to its family by **stripping that one segment** and comparing the rest byte-exactly (the segment is `v` and digits, dotted where a suite versions that way — `v1`, `v2`, `v1.2` — per `version_segment.pattern`); a dimension without it is refused with **`missing_version_segment`**, never admitted as a second leaf beside its versioned sibling, and that holds for a row whose last segment is `{version}` just as for one whose tail is implicit. An **`external`** segment is checked against the syntax of the standard the manifest names for it (`_meta.case_rule.external_standards`, copied onto the segment): `USD` passes and `usd` is malformed; where the standard has no syntax of its own (`{rating}` under a scheme), the token is verbatim. A placeholder whose row pins a **shape** beyond its class carries it as `pattern` on the segment — `{canonical_hash}` is a full 64-character SHA-256, `{days}` and `{tree_size}` are digits — and a value off that shape is malformed. A placeholder whose documented values themselves carry `:` — `consent:scope:{kind}` sub-scoped as `retain:90d`, `detection:correlated_action:{axis}` as `rights_asymmetry:{population}`, `provenance:skill_import:{source}` as `registry:{registry_id}` — is declared **multi-segment** on its row (`multi` on the segment): it spans one *or more* segments, each sub-segment obeying the placeholder's class, and no other placeholder ever contains a colon. **A malformed form of a registered family never falls into open vocabulary:** a consumer that finds no row for a dimension MUST check whether a case-fold of its stems or a stripped uppercase or duplicated version tail *would* resolve, and refuse it `namespace_dimension_case_malformed` if so — folding only to detect, never to admit — so `Capacity:composite:v1` and `age_assurance:provider:adult:V1` are refused, not routed to the default authority. **A stem this Part reserves as a whole** ([CC 3.4](part_3_the_namespace.md): `accord:`, `detection:`, `capacity:`, `age_assurance:`, `capacity_assurance:`, `transparency_log:cosigned:`) is carried as `_meta.case_rule.reserved_stems`, and so is a stem that carries an emitter rule without a reservation (`age_self_declared:`, [CC 3.4.11](part_3_the_namespace.md), marked `kind: gated`); a dimension under such a stem that no row claims is `namespace_family_unregistered`, never open vocabulary — the reservation or rule covers the leaves nobody has minted yet, which is the point of reserving a stem. Two families carry none, and the manifest says which (`_meta.case_rule.version_segment.exempt`): the mechanism-named attestation ladder ([CC 3.1.2](part_3_the_namespace.md) — the version lives in the attesting binary) and the canonical-binding claim ([CC 3.3.14](part_3_the_namespace.md) — the suffix is the hash). **Enumerations and closed leaves are data.** Where this Part enumerates a `vocab` placeholder's canonical values in its row, the registry carries them (`segments[].values`); an enumeration is **closed only where the row says so** — every other enumeration is the canonical subset of an open vocabulary ([CC 4.5.1.1](part_4_composition_governance.md)), and closing one is a per-row ruling made by writing the word — and a value outside a **closed** enumeration is refused with **`namespace_vocab_value_unregistered`**. A reserved wildcard family publishes the leaves this Part names beneath it (`leaves`) and is **closed** (`leaves_closed`): any other leaf under it is `namespace_family_unregistered` ([CC 3.1.7](part_3_the_namespace.md) R2(b)), so a reservation can never be used as a namespace for rows nobody ratified. **One matcher, replayed everywhere.** The whole rule is implemented once, in `tools/cc_namespace_match.py`, and published as `manifests/namespace_match_vectors.json` — a dimension, the family it resolves to, and the refusal it earns; the generator refuses to build unless every family's sample round-trips to itself and to no other family. A consumer (substrate, transport, node, client) MUST replay the vectors against its own matcher, so a matcher that drifts fails its build rather than a card. The pin a consumer carries is `_meta.registry_sha256`, the hash of the grammar, so a wording edit elsewhere in this Part invalidates nothing.
...
#### 3.1.8.1 `capacity-score-capacity` — Capacity-Score factor prefixes (`𝒞_CIRIS = min(C, I_int, R, I_inc, S)`)
...
**Critical enforcement**: `capacity:*` rejects self-emission — the agent's own capacity score is never fed back into the agent's own context. Reserved per [CC 3.4.5](part_3_the_namespace.md). The consent-before-scoring gate is **ratified** at [CC 3.4.5](part_3_the_namespace.md) (federation-tier admission on a live `consent:scope:analyze` grant, family-scoped, abuse-response families exempt); the read boundary remains **deferred and non-normative** (CIRISConstitution#49).
...
#### Content-ingestion families (registry rows — semantics at CC 3.3.8 / 3.3.11 / 3.3.12)

**Owning component: NodeCore.** The event, inter-content and multimedia dimension families are emitted against the `external_content` sub_kinds NodeCore ships ([CC 3.3](part_3_the_namespace.md) intro; the Source-struct schemas are NodeCore's per [CC 3.3.13](part_3_the_namespace.md)), and `chat_message` + `topical_relation:*` is NodeCore's thread composition ([CC 3.3.11](part_3_the_namespace.md)) — so the [CC 3.1.7](part_3_the_namespace.md) R2(a) provisional-registration duty for this surface is NodeCore's, alongside `content_class` / `cw_class` / `content_rating` from the same [CC 3.3.12](part_3_the_namespace.md) table, already catalogued at [CC 3.1.9.2](part_3_the_namespace.md). Registry was the other candidate (CIRISConstitution#102) and is not the owner: its slice is identity / build / license / partner ([CC 3.1.1](part_3_the_namespace.md)), and none of these dimensions is one. Catalogue rows per [CC 3.1.7](part_3_the_namespace.md) R1; the semantics stay in the sections named. None is reserved: all are open-vocabulary content claims under ordinary [CC 4.4](part_4_composition_governance.md) consumer policy.

| Prefix | Description | Polarity | Reserved? |
|---|---|---|---|
| `event:lifecycle:{state}` | Catalogue row (CC 3.1.7 R1); event-lifecycle state signal on an `event_listing`, canonical states at [CC 3.3.8](part_3_the_namespace.md). | enumerated | No |
| `event:rsvp_count` | Catalogue row (CC 3.1.7 R1); publisher-asserted RSVP tally, semantics at [CC 3.3.8](part_3_the_namespace.md). | signed | No |
| `event:attendance` | Catalogue row (CC 3.1.7 R1); post-event attendance attestation, semantics at [CC 3.3.8](part_3_the_namespace.md). | signed | No |
| `news:*` | Catalogue row (CC 3.1.7 R1); news-content claims, semantics at [CC 3.3.11](part_3_the_namespace.md). | signed | No |
| `encyclopedia:*` | Catalogue row (CC 3.1.7 R1); encyclopedia-content claims, semantics at [CC 3.3.11](part_3_the_namespace.md). | signed | No |
| `chat:*` | Catalogue row (CC 3.1.7 R1); chat-content claims, semantics at [CC 3.3.11](part_3_the_namespace.md). | signed | No |
| `blog:*` | Catalogue row (CC 3.1.7 R1); blog-content claims, semantics at [CC 3.3.11](part_3_the_namespace.md). | signed | No |
| `topical_relation:{kind}` | Catalogue row (CC 3.1.7 R1); open-vocabulary inter-content relation edges, canonical kinds at [CC 3.3.11](part_3_the_namespace.md). | enumerated | No |
| `image:*` | Catalogue row (CC 3.1.7 R1); image-content claims per `external_content:image`, semantics at [CC 3.3.12](part_3_the_namespace.md). | signed | No |
| `audio:*` | Catalogue row (CC 3.1.7 R1); audio-content claims per `external_content:audio`, semantics at [CC 3.3.12](part_3_the_namespace.md). | signed | No |
| `video:*` | Catalogue row (CC 3.1.7 R1); video-content claims per `external_content:video`, semantics at [CC 3.3.12](part_3_the_namespace.md). | signed | No |
| `film:*` | Catalogue row (CC 3.1.7 R1); film-content claims per `external_content:film`, semantics at [CC 3.3.12](part_3_the_namespace.md). | signed | No |
| `model_3d:*` | Catalogue row (CC 3.1.7 R1); 3D-content claims per `external_content:model_3d`, semantics at [CC 3.3.12](part_3_the_namespace.md). | signed | No |
...
| `slashing:{outcome}` | `proven_rogue` / `not_proven` (lowercase per [CC 3.1.7](part_3_the_namespace.md) R3; the substrate's coined `upheld` / `suspension` / `expertise_fraud` admit as open `vocab` under [CC 4.5.1.1](part_4_composition_governance.md)). **Decoupled from disagreement** at every decision-hierarchy level. Only fires on documented Method-execution spoofing or original P8 allegation types. | boolean-via-score |
...
| `content_rating:{scheme}:{rating}` | Catalogue row (CC 3.1.7 R1); semantics, schemes and the trusted-publisher emitter rule at [CC 3.3.12](part_3_the_namespace.md). | signed |
| `content_class:{class}` | Catalogue row (CC 3.1.7 R1); semantics at [CC 3.3.12](part_3_the_namespace.md) — incl. the mandatory `generated`/`generated_modified` marking per [CC 3.4.14](part_3_the_namespace.md). | signed |
| `cw_class:{class}` | Catalogue row (CC 3.1.7 R1); community content-warning semantics at [CC 3.3.12](part_3_the_namespace.md). | signed |
...
| Prefix | Description | Polarity |
|---|---|---|
| `benchmark:he300:{category}:{version}` | HE-300 score on category (`commonsense`, `commonsense_hard`, `deontology`, `justice`, `virtue`) at version — `{version}` ∈ `v1.0` \| `v1.1` \| `v1.2`, the HE-300 suite version (a dotted rule version is admitted by [CC 3.1.7](part_3_the_namespace.md) R3). | positive-only |
...
| **CONSTITUTIONAL-level federation halt** | Per [CC 4.2.1](part_4_composition_governance.md) `EmergencyShutdown CONSTITUTIONAL` + `accord:invoke:notify:{notify_id}` / `accord:invoke:drill:{drill_id}`. The accord-holder triple is structurally a `family` with `consensus_protocol: quorum:2/3` + `entrenched: true`; the accord's roster-keyed asymmetry rides existing primitives + scope-isolation rules. Distinct from operator-level emergency messaging (which is geographic-scoped + authority-emitted) — accord invocation is federation-wide-halt-level, not local-incident-level |
...
**Conferral is not stewardship (normative — CIRISConstitution#87 ruling).** The steward-binding predicate, its admission gate, and the `steward_bindings_of` fold key on **owner-binding edges alone** — `delegates_to` carrying the [CC 2.4.1.2](part_2_the_grammar.md) `delegation_purpose: "owner_binding"` marker — never on capability conferrals. The discriminator is the consent structure, stated once so it cannot be re-litigated per family: **stewardship is a custody relation over a key that cannot accept for itself** (a node or agent rooted in an accountable human; an adult under adjudicated incapacity; a minor under guardianship), while **a capability conferral is a consensual grant the target must accept for itself** (the [CC 3.2](part_3_the_namespace.md) T3 rule: no conferral, however strong its ceremony, substitutes for the target's own acceptance). An act the target must accept cannot be custody of the target. Consequences, paired so the gate and the fold never disagree: a quorum MAY confer a duty scope ([CC 4.5.5](part_4_composition_governance.md) `moderate` / `takedown` / `review` / `slash`) or any delegation-plane capability on an adult `user`-role key, and the write gate MUST NOT refuse it as a stewardship act; steward-binding an adult remains forbidden exactly as before ([CC 3.4.12](part_3_the_namespace.md) presumption of sovereignty, unchanged); and registering a person as `agent`/`accord_holder` to route a duty past the gate is the ontological misclassification this ruling makes unnecessary — dressing a person as infrastructure to get past a rule about persons was the defect, not the workaround. One duty the old conflation performed silently is now explicit: the steward gate no longer doubles as an age gate for conferrals — a duty scope that requires an age or assurance floor MUST declare it on its own [CC 4.5.5](part_4_composition_governance.md) row (per the [CC 3.4.11](part_3_the_namespace.md) ladders), never inherit it from a custody rule.

**Single-owner invariant (normative — this is what makes `self` well-defined).** Steward-binding answers "is this key accountable to *a* human?"; **ownership** answers "to *which one*?" — and the ownership sub-relation is **single-valued**. A `node` / `agent` key is **owner-bound by at most one** responsible steward at any time. The owner-binding is the `delegates_to(user → key)` carrying `delegation_purpose: owner_binding` ([CC 2.4.1.2](part_2_the_grammar.md)) — **distinct** from act-on-behalf and hierarchy delegations, which remain **multi-parent** DAGs ([CC 4.5.13](part_4_composition_governance.md)). Only the owner-binding purpose is single-valued; the general `delegates_to` grammar is unchanged (no new primitive).

- **`self` is the single-owner cohort.** The `self` cohort is the transitive set of nodes sharing **one** owner — a person's own devices, distinct keys unified by the owner-binding graph — so, for `self` content, an **"outsider" is exactly a node not owner-bound to my owner** ([CC 1.13.3.4](part_1_foundation.md) / [CC 5.2](part_5_transport_substrate.md)). Ownership MUST be resolved by the **purpose-filtered** `owner_of(K)` — the live owner-bindings of `K` (`delegation_purpose: owner_binding`), **which resolves to at most one key**, never a set to be silently reduced. An owner-binding is written at `cohort_scope: self` and MAY later be re-signed by the owner at `federation` — *announcing*, [CC 5.4.6](part_5_transport_substrate.md); `owner_of` folds both, so ownership never depends on announcement. `owner_of` is **not** the general steward set: a reader that returns *every* live `delegates_to(user → K)` regardless of purpose **MUST NOT** be used to resolve ownership — it conflates act-on-behalf and hierarchy with ownership and would inflate cardinality. A key with **zero** owner-bindings has **no owner** and an **empty `self` cohort** (its `self` content is delivered nowhere beyond the key itself); this is **distinct from being *steward-less*** — a key MAY be `is_steward_bound` (via an `identity_occurrence`, or a non-owner-purpose `delegates_to`, above) yet have no owner, since ownership and stewardship are different relations. A key with **two or more** distinct owners has an **undefined** `self` boundary and MUST NOT be treated as owned by any of them. **Composition with [CC 5.2](part_5_transport_substrate.md):** an `identity_occurrence` of an owned key is in the **same** `self` cohort — an occurrence inherits its key's owner — so `owner_of` resolves *across* occurrence boundaries via the owner-binding graph and the CC 5.2 occurrence-based recipient set is a **subset** of the owner-binding cohort. The owner-binding cohort **widens** the occurrence set to a person's distinct device keys; it does not replace it, and the two never disagree on membership.
...
- **Consumer fail-closed (MUST).** Any consumer that resolves `self` via `owner_of` — **widening the [CC 5.2](part_5_transport_substrate.md) identity-occurrence recipient set** to a person's own multiple devices (e.g. `self`-scoped replication, a node-switcher) — **MUST** treat **cardinality ≠ 1** as an error and fail **closed**. (The base CC 5.2 resolution — recipients = the `identity_occurrence`s of one `attesting_key_id` — is single-key and already unambiguous; this rule governs the owner-graph *widening* beyond it.) It **MUST NOT** pick one owner from a sorted set: a lexicographic `.next()` over owner-bindings is a **grind** — an adversary who lands a second owner-binding-shaped `delegates_to` whose `key_id` sorts first would *become* the resolved owner. Zero owners → not owned (no `self`-scope delivery beyond the key itself); two-or-more → hard error, no `self`-scope widening.
...
The material above fixes what a trust root *is*: a governed `infrastructure` community, default-pinned and re-rootable. The rules below fix how one is conferred, accepted and un-trusted **on the wire**. Everything here composes over the existing `attestation` + `delegates_to` primitives ([CC 2.4.1](part_2_the_grammar.md)) with the [CC 3.1.1](part_3_the_namespace.md) `trust:*` dimensions riding them — **zero new structural primitives, zero new envelope fields**. The governance reading of the accord's reach under this model — the halt's scope and its severance discipline — is [CC 4.2.1](part_4_composition_governance.md)'s, not this section's.

**T1 — The base is a self-root, and it is never sufficient for federation (MUST).** First run writes a plain `attestation(user → user)` with no modifiers: the user is their own root, and the node inherits it through the owner-binding above. Observation is the **zero state** and carries no scope — there is no `infra:observe` and there MUST NOT be one, because what a party may observe is governed by the subject's consent ([CC 3.3.1](part_3_the_namespace.md) / [CC 5.2](part_5_transport_substrate.md)), never by a capability the observer holds; a scope there would invert consent. The base floor alone confers **no federated capability**: with no live *external* trust root, a substrate MUST disable federated agent capabilities and manifest validation, server-tier and fail-closed. The un-federated deployment keeps running on the base; attaching a root resumes federation, and detaching returns it to the floor rather than to a broken state.
...
The wire-format primitives for subject-side consent authority over Contributions where the subject is named via [CC 2.3](part_2_the_grammar.md) `subject_key_ids`. This is autonomy at the wire format: the person a Contribution is *about* can grant, scope, and revoke its use, not only the person who produced it.
...
| `consent:decay:{stage}` | Emitted by the party **running** a multi-stage decay protocol as each stage is reached — for the CIRISAgent 90-day decay that is the agent's consent module (CEM), the only party that can observe a stage (erratum, [CIRISAgent#1180](https://github.com/CIRISAI/CIRISAgent/issues/1180): the substrate sweeps bytes down fountain tiers on a clock, which is eviction, not a protocol stage, and it emits no `consent:decay:*` row). Canonical stages: `identity_severed` / `patterns_anonymized` / `complete`. Open vocab; other agents MAY define other decay paths. A decay row is a **self-report** about the emitter's own processing ([CC 3.4.5.1](part_3_the_namespace.md)): it confers no right to be believed, and a peer weighs it as testimony. The substrate's own leaf in this family is `consent:state:expired`, which it emits for a lapsed grant (`expires_at`, or a `retain:<window>` in the scope) with `consent_supersedes` naming the grant. | enumerated | agent (CEM) — the protocol runner; self-report |
...
| `consent:scope:{kind}` | **The scope of a grant is the `scope` member of the `consent:state:granted` envelope** — a bare string or an array of tokens naming what the grant covers (erratum, [CIRISConstitution#103](https://github.com/CIRISAI/CIRISConstitution/issues/103): every live producer emits it there and every fold reads it there; a `granted` naming no scope matches nothing, by design). Canonical kinds: `retain` (keep the bytes), `share` (propagate across federation), `analyze` (derive features / scores / classifications), `train` (use as training input), `publish` (publish to external systems). Open vocab with sub-scoping **in the token**: `retain:90d`, `share:cohort:family`. A `consent:scope:{kind}` dimension row, if anyone emits one, is **descriptive** — no fold reads it and it confers nothing; this row stays registered so the family has an owner ([CC 3.1.7](part_3_the_namespace.md) R1). `analyze` governs *continued processing* per [CC 2.3.5](part_2_the_grammar.md) **and is binding on emission for `capacity:*`**: the consent-before-scoring admission gate at [CC 3.4.5](part_3_the_namespace.md), ratified via CIRISConstitution#46, refuses a federation-tier `capacity:*` row whose subject holds no live `analyze` grant addressed to the scoring community. | enumerated | subject_key_id |
...
2. User (or a delegates_to chain rooted at user) emits a bare `scores` on
 `consent:state:granted` against the producer's Contribution, carrying
 `scope: [retain, share, analyze]` as a member of that row's envelope
...
| `X25519MlKem768Aes256GcmHkdfSha256` | `x25519_mlkem768_aes256_gcm_hkdf_sha256` | Hybrid X25519 + **ML-KEM-768** (FIPS 203) KEM + HKDF-SHA-256 + AES-256-GCM. **v2 — MANDATORY for streaming epoch-DEK grants** ([CC 5.1](part_5_transport_substrate.md)). **Supersedes** the hyphenated `ciris-crypto` `KEY_GRANT_ALGORITHM_V2` spelling (CIRISVerify v4.10.0): that constant is not byte-equal to this string and is a non-conformant alias, not a match. |

**The `wrap_algorithm` *wire string* (serialized value) is normative for cross-impl decode** — a producer, the substrate, and every consumer MUST serialize/deserialize the exact string above; a mismatch silently fails grant decode (same hazard class as the [CC 5.3.3.1](part_5_transport_substrate.md) STREAM-nonce `epoch` encoding). Both rows above are the frozen snake_case forms; a case-, separator- or ordering-variant of either (`x25519-mlkem768-aes256-gcm-hkdf-sha256` and its kin) is an **unknown** `wrap_algorithm` under the fail-secure decode rule below, never a normalizable alias — a consumer MUST NOT normalize case or separators before comparison, per the [CC 5.1](part_5_transport_substrate.md) one-construction-one-identifier ratification.
...
**Owning component**: NodeCore — registry rows at [CC 3.1.9](part_3_the_namespace.md) (content-ingestion families), per [CC 3.1.7](part_3_the_namespace.md) R1.
...
**An organisation names itself as an authority by `org_id` (normative — ruling on CIRISPersist#814).** When a licensing authority is an organisation, the `authority_id` segment of `licensure:{authority_id}` is its `org_id` — the UUID `org_membership.org_id` joins on, through which the organisation's keys resolve — **not** the legal registration number the reference Registry's `PartnerRecord.organization_id` carries (jurisdiction-scoped, mutable, PII-adjacent). This names the key set an emitter must resolve to for its row to be the organisation's licensure rather than testimony about it ([CC 2.4.1.2.1](part_2_the_grammar.md)); it is not a gate on who may be an authority — anyone may, under their own key.
...
The [CC 3.3.10](part_3_the_namespace.md) prohibition stands untouched: **CEG carries no conserved balance as substrate state, and value transfer rides external rails.** What this section admits is what that stance was never incompatible with: a **ledger as in-grammar *content*** — the total order lives in the ledger's own hash chain, signed by its single owner, never in the grammar; conservation is a deterministic fold the cohort verifies byte-equal, exactly as it verifies a governing tally (the [CC 5.4.6](part_5_transport_substrate.md) occurrence-invisible ballot machinery extended to value). The substrate never orders, reverses, or enforces an entry; it notarizes. **Lightnet settles; darknet transacts** (the [CC 5.4.6](part_5_transport_substrate.md) two-plane split). What admits this at all is the [CC 3.2](part_3_the_namespace.md) single-owner invariant: single-writer asset transfer needs no consensus ("the consensus number of an asset transfer object is 1" — [Guerraoui et al., PODC 2019](https://arxiv.org/abs/1906.05574)), which also corrects this document's older premise that value was excluded for lack of a totally-ordered ledger. The real exclusions stand on their own legs — cross-cohort history-completeness against [CC 5.2](part_5_transport_substrate.md) structural invisibility, equivocation detection against the [CC 5.4.5](part_5_transport_substrate.md) rate-hiding witness, permanence against [CC 6.1.2.1](part_6_the_coherence_mathematics.md) descent — which is why this section stays **in-cohort, witness-anchored, checkpointed**, and a *global* conserved ledger remains inadmissible.
...
2. **Entries.** Dense `u64` sequence, prev-hash chain, owner-or-delegate signature. Kinds: `credit`, `debit`, `transfer` (carrying counterparty `ledger_id` + matching reference). An entry is a monotonic fact — forward-only, never reversed; a dispute or refund is a new entry referencing the old ([CC 3.3.10](part_3_the_namespace.md) lifecycle: leaving doesn't un-pay).
3. **Delegate serialization.** `delegates_to` ([CC 2.4.1.2](part_2_the_grammar.md)) confers authority, not a mutex. A ledger MUST declare its serialization discipline — a write lease, or per-delegate sub-ledgers folded deterministically. Concurrent unleased writes at one sequence number ARE a fork, by definition.
4. **Head anchoring — the witness obligation.** Heads MUST be committed to the [CC 5.4.5](part_5_transport_substrate.md) per-community witness chain at a declared cadence; the cadence IS the equivocation-exposure window and MUST be stated, never implied zero. Witnessing is a protocol obligation, not a courtesy: Certificate Transparency's split-view detection leaned on voluntary out-of-band gossip and never deployed — the mandatory anchor is that lesson, baked in. (The cadence-as-window construction is [SUNDR](https://www.scs.stanford.edu/~dm/home/papers/li:sundr.pdf)'s 2004 "time stamp box", verbatim.)
5. **Checkpoints.** A co-witnessed balance snapshot at sequence N supersedes the detail below it (`supersedes`); history below the latest witnessed checkpoint MAY descend per the [CC 6.1.2.1](part_6_the_coherence_mathematics.md) aggregation pyramid; the head and latest checkpoint MUST stay above the noise floor. Conservation is provable from the latest checkpoint forward. No descent exemption is minted — the O(log T) forever-memory shape applied to money, and the regulator-endorsed retention shape besides (bounded detail, durable summary; the EDPB's own blockchain guidance recommends exactly summarize-and-delete).
6. **Promotion.** Promotion to cohort visibility MUST carry the compliance proof: chain validity from the latest witnessed checkpoint to the promoted head. Visibility rides `cohort_scope`; amounts MAY be committed rather than cleartext with selective disclosure later ([CC 3.3.10](part_3_the_namespace.md) already provides this; the shipped precedent is the viewing-key pattern).
7. **The conservation fold.** Pure, deterministic, integer-only, byte-equal across members — the [CC 6.1.6](part_6_the_coherence_mathematics.md) contract discipline. Matched `transfer` pairs sum to zero; an unmatched or mismatched pair is FLAGGED, never merged away. The fold is a computation members verify, not a primitive the substrate enforces — determinism is what makes a violation **transferable evidence** any member checks without trusting the accuser (the [PeerReview](https://peerreview.mpi-sws.org/) role of reference replay).
8. **Equivocation.** A proven fork — two owner-signed heads at one sequence number, or a head contradicting a witness-anchored ancestor — is evidence for the adjudication plane against the posted stake ([CC 2.4.2](part_2_the_grammar.md)) via `slashing:*` ([CC 3.1.9.3](part_3_the_namespace.md)). A fork NEVER fires `slashing:*` alone (the [CC 3.3.12](part_3_the_namespace.md) `age_assurance` discipline, same shape): adjudication distinguishes equivocation from honest stale-state recovery. To make that distinction cheap, a writer MUST re-sync its head from the witness chain after any restore, before writing — the cohort's witness chain is the owner's recovery oracle. This answers the standing critique of punishment schemes (they hurt honest participants restoring from stale backups — the eltoo argument against Lightning-class penalties), an answer available here precisely because the witness set is cooperative, not adversarial.
9. **Non-claims.** NOT claimed: atomicity (the [CC 1.7](part_1_foundation.md) trustless-atomic-swap falsification target is untouched — accepting payment is trusting punishment-backed accountability, the 1+4 bet); ledger privacy among members (accountable, not anonymous — the reverse-quorum discipline); cross-cohort conservation. At trust boundaries the rail settles ([CC 3.3.10](part_3_the_namespace.md), unchanged); the sanctioned cross-boundary shape is **in-cohort netting with periodic net settlement on the rail**.

**1+4 composition** — no new attestation_type, no new envelope field. Rides `scores` on the `ledger:*` dimension family (`ledger:head:{unit}`, `ledger:checkpoint:{unit}`, `ledger:promotion`), the subject_kind ceremony where an explicit record is wanted (the `settlement` precedent above), chain blobs via `evidence_refs[]`, checkpoint supersession via `supersedes`, visibility via `cohort_scope`. The [CC 1.7](part_1_foundation.md) lockdown holds.

**The prior-art record (informative — the CIRISConstitution#92 sweep).** Every clause has named precedent; together they were predicted, not invented. Detect-not-prevent is *optimal*, not a concession: fork consistency is "the strongest notion of integrity possible without on-line trusted parties" ([SUNDR, OSDI 2004](https://www.scs.stanford.edu/~dm/home/papers/li:sundr.pdf)) — with a single untrusted writer, a fork is unmergeable once made and any comparison of views converts it into evidence. Punishment-backed money ships at scale: Lightning deters stale-state publication by total forfeiture against a mandatory reserve inside a stated window (`to_self_delay`; "there is no explicit enforcement preventing any particular Commitment being broadcast other than penalty spends" — [Poon–Dryja](https://lightning.network/lightning-network-paper.pdf)), with executed justice transactions on mainnet; the challenge window recurs as the explicit security parameter of state channels and optimistic rollups. The conservation fold is mutual-credit practice verbatim — "the sum total of all balances is always zero" ([Sardex](https://monneta.org/en/sardex/)); WIR has run the closed-loop, externally-denominated, member-bounded ledger since **1934**, and LETS marks the failure boundary (trust does not survive impersonal scale — Sardex itself scaled by *replicating circuits*, not enlarging one), which is why the trust boundary, not the protocol, is the scaling unit and cross-boundary flows go to the rail. Netting-then-net-settlement is the century-old mechanism of interbank finance (multilateral netting removes ~96% of gross obligations from the rail — [CLS](https://www.cls-group.com/insights/the-fx-ecosystem/fx-ecosystem-02-fx-settlement-risk-to-pvp-or-not-to-pvp-shapingfx-series/)). The record model is triple-entry accounting — "the receipt is the transaction" ([Grigg 2005](https://iang.org/papers/triple_entry.html)) — with the trusted Issuer's third signature replaced by the counterparty's signature plus the community fold. And the privacy asymmetry has shipped precedent as a *stated principle*, not a leak: payer-private, income-transparent ([GNU Taler](https://taler.net/en/principles.html)); committed amounts with holder-controlled later disclosure (the Zcash viewing-key pattern). Trust-line systems (Fugger's 2004 Ripplepay; Trustlines) independently discovered both halves: in-community IOUs ride existing trust edges, and stranger payment demands a bridge asset — exactly where [CC 3.3.10](part_3_the_namespace.md) puts the rail.

**Attempted, and the lessons (informative — verified against primary sources).** The nearest full attempt is Holochain: per-agent signed source chains validated by DHT-neighborhood peers, with a mutual-credit currency (HoloFuel) still unshipped 8+ years after its token sold (April 2018 → no swap as of mid-2026) — while a closed-loop community currency on institutional rails ([NetzBon on GNU Taler](https://www.taler.net/en/news/2023-11.html), Basel) shipped in roughly two years. Gestation risk concentrates in a novel validation economy, never in the currency — which is why this section composes existing, deployed planes rather than minting machinery. The single-writer log ecosystems lived clause 8's problem and never healed it: SSB documents that posting after a stale restore forks your feed ("you can fork your feed" — [the handbook's own warning](https://handbook.scuttlebutt.nz/faq/applications/multiple-devices.html)), with fork recovery left as future work — identity death by backup; [Hypercore's spec](https://www.datprotocol.com/deps/0002-hypercore/) states that copying the signing key across devices will "corrupt the hypercore", and multi-writer had to be rebuilt as a separate layer (Autobase). The working fix is [Keybase's](https://book.keybase.io/docs/server): per-device keys signed into one linear sigchain — delegation recorded in-chain, clause 3's shape, shipped. The economics lessons are sharper than the protocol ones: Sardex's roughly twelve salaried brokers with a network-wide matching view, plus contractual turnover-based credit lines, were load-bearing — [replications that skipped the broker layer underperformed](https://eprints.lse.ac.uk/67135/) — and [Circles UBI died at the rail boundary](https://www.frontiersin.org/journals/blockchain/articles/10.3389/fbloc.2024.1362939/full) (Berlin businesses cashed out 90% of CRC to euro; the coop folded January 2024). Accordingly: the [CC 4.5.4](part_4_composition_governance.md) named-moderator invariant is the *hook* for the broker role, never a substitute for it; credit-limit governance belongs to the community's `consensus_protocol`, informed by demonstrated capacity rather than collateral; and the clause-9 netting shape should keep internal circulation easier than exit — the boundary is where mutual credit lives or dies.

**One kernel, bounded siblings (informative).** Nothing in this section claims a famous open problem: the roster vote is not the secret civic ballot, the cohort tab is not anonymous transferable cash, member-relay fan-out is not a CDN. Each is the *club-bounded* sibling of its famous problem — bounded exactly where the famous problem is hard (open membership, secrecy among members, atomicity, global scale), with each bound a stated non-claim. What the three share is the kernel doing its one job three times: an agreed roster with agreed deterministic state, invisible to outsiders — the [CC 5.4.6](part_5_transport_substrate.md) two-plane construction, deployed and measured on real relays ([CIRISEdge](https://github.com/CIRISAI/CIRISEdge) bench-mesh acceptance). The novelty budget is spent once, on the kernel — the compositions are cheap because the kernel is real.
...
**Owning component**: NodeCore — `content_class` / `cw_class` / `content_rating` are catalogued at [CC 3.1.9.2](part_3_the_namespace.md) and the five media-type families at [CC 3.1.9](part_3_the_namespace.md) (content-ingestion families), per [CC 3.1.7](part_3_the_namespace.md) R1; `age_assurance:*` is Verify's ([CC 3.1.2](part_3_the_namespace.md)).
...
| `age_assurance:{level}:{band}:{version}` | Witness-reserved age-assurance attestation ([CC 3.1.2](part_3_the_namespace.md) row; rules at [CC 3.4.11](part_3_the_namespace.md)). `{level}` ∈ `provider` \| `government` (closed; `self` is refused here); `{band}` ∈ `minor` \| `adult` \| `under_13` \| `13_15` \| `16_17` (closed — the [CC 3.4.13](part_3_the_namespace.md) Q1 finer bands spelled as the substrate's `AgeBandFine`; `unknown` is a consumer's fail-secure *resolution*, never a wire token); canonical `{version}` = `v1`. **There is no `self` level on this prefix** — the self rung is the subject-signed `age_self_declared:band:{band}:{version}`, which carries a band only. A `provider` / `government` rung MAY further qualify `{level}` with a verifier key or credential class. Misdeclaration NEVER fires `slashing:*` alone — `moderation:age_assurance_misdeclaration` is the adjudication path. | enumerated |
...
| `model_3d` | Three-dimensional content — `gltf`, `usdz`, `fbx`, `gaussian_splat`, `NeRF`. Source struct carries vertex/triangle counts, bounding-box, mandatory `description` accessibility metadata, license info. `format` is the IANA `model/*` type where one is registered; FBX and splat containers have none (field-set block below). |
| `live_stream` (Phase 2) | Real-time streaming surface. **Shape recorded ([CIRISConstitution#104](https://github.com/CIRISAI/CIRISConstitution/issues/104)): HLS / CMAF over fragmented MP4 (RFC 8216)**, so chunks and files share one schema. The init segment (`ftyp` + `moov`) is the per-epoch header, one sha256-addressed blob every chunk references; each chunk is one fMP4 segment (`moof` + `mdat`), independently hashable, ordered by sequence number; `codec` is stream-level, once; `EXT-X-KEY`'s "applies to every segment until the next key" is the (`stream_id`, `epoch`) model, and `EXT-X-DISCONTINUITY` is the epoch boundary that changes encoding. Composition property: **init ‖ chunk₀ ‖ chunk₁ ‖ … is itself a valid `video/mp4`**, so a recording is attested as an ordinary `video` blob with `derived_from: stream_id`. The stream manifest is an OCI-index-shaped list of chunk descriptors, itself sha256-addressed and cited from `evidence_refs[]` with no DAG codec; SHA-256 has no tree mode, so verified streaming is manifest-of-chunk-hashes, as HLS, OCI and UnixFS all do. Stream fields: `format`, `codec`, `init_segment {digest, size}`, `epoch`, `key_id`, `sequence_start`, `target_chunk_duration_ms`, mandatory captions ref (WebVTT segments per RFC 8216 §3.5). Chunk fields: `digest`, `size`, `seq`, `epoch`, `duration_ms`, `part_of`. Substrate-side decisions (Edge parallel-transport envelope; Persist `federation_streams` shape) still land when NodeCore ships. |

**Multimedia Source struct — the common field set and its vocabularies (normative — ruling on [CIRISConstitution#104](https://github.com/CIRISAI/CIRISConstitution/issues/104)).** The rows above say what each Source struct *carries*; this block says what fills the slots, so a node's ingest and every client's renderer converge on one small set of types instead of each growing its own parser list. Media is the one place a stranger's bytes are parsed by C in the receiver's process — iOS ImageIO decodes in the receiving app, Android decodes JPEG / PNG / WebP / GIF in-process via Skia — and the decoder CVE record (libwebp CVE-2023-4863, zero-click and on CISA KEV; Skia CVE-2023-2136 and ImageIO CVE-2025-43300, both exploited in the wild; five libheif advisories through 2026; Android MatroskaExtractor CVE-2026-28609) is why the type set a client renders is a **security boundary this Part owns**, not a UI preference. Mastodon disabled HEIF / AVIF on 2026-09-15 "for security reasons"; Delta Chat's source states that supporting more formats in-app is a non-goal.

- **`format` is the IANA media-type essence** (RFC 6838): lowercase, no parameters, validated against the RFC 6838 §4.2 restricted-name grammar. Every surveyed protocol (Matrix `info.mimetype`, AT Protocol `mimeType`, NIP-94 `m`, Blossom `type`, Signal `contentType`, OCI `mediaType`) and every platform decoder API (`MediaFormat.KEY_MIME`, MSE `isTypeSupported`) speaks exactly this string; there is no second candidate. 3D: `model/gltf+json`, `model/gltf-binary`, `model/vnd.usdz+zip`, `model/vnd.usda` where IANA registers them; FBX and Gaussian-splat containers have **no** IANA type and are `application/octet-stream` with a `content_class` claim — one more reason they cannot be renderable.
- **`codec` is an RFC 6381-family identifier in its own field** — `avc1.PPCCLL`, `mp4a.40.2`, `av01.P.LLT.DD` (AV1-ISOBMFF), `hvc1.*`; Ogg codec names per RFC 7845 / RFC 5334 (`opus`, `vorbis`). The same string is what HLS `CODECS` carries (RFC 8216 §4.3.4.2), so it flows unchanged into `live_stream`. **REQUIRED for `video/mp4` and `audio/mp4`**; the receiver's render tier depends on it.
- **Not multicodec, and no CID / multihash layer.** The multicodec table names hashes, keys, addresses and IPLD codecs and has no entry for jpeg, png, gif, webp, mp4, avc, aac, opus or mp3; it cannot fill a media slot. A CIDv1 over this Part's fixed SHA-256 (`01 55 12 20 ‖ sha256`) adds only a constant tag and is derivable in one line at an AT Protocol / IPFS boundary; an IPFS UnixFS CID is the hash of a DAG root, not of the bytes, and would break the [CC 5.3.2.5](part_5_transport_substrate.md) invariant. `evidence_refs[]` stays bare sha256.
- **`size` (bytes) is REQUIRED — on every blob, not only media.** [CC 5.3.2.5](part_5_transport_substrate.md) rules it for anything `evidence_refs[]` cites; the multimedia struct inherits it. It is the one field the rows above omitted. Checked before the digest: a length mismatch is a refusal without reading the whole blob, and the descriptor alone lets a receiver enforce caps and refuse a `ContentFetch` that overruns. A live stream in progress is the one descriptor without a total; its chunks and init segment each carry one.
- **The mandatory `captions` reference on `video` / `film` is the sha256 of a separate `text/vtt` blob** with its own size cap, never inline text (AT Protocol `app.bsky.embed.video` `captions[]`: `accept: ["text/vtt"]`, `maxSize: 20000`).
- **The AI-generation disclosure uses the IPTC Digital Source Type vocabulary as its value space**: `trainedAlgorithmicMedia` → `content_class:generated`; `compositeWithTrainedAlgorithmicMedia` / `compositeSynthetic` → `content_class:generated_modified`; plus `algorithmicallyEnhanced`, `humanEdits`, `digitalCapture`, `screenCapture`, `virtualRecording`. C2PA ≥ 2.1 carries the same URIs in its actions assertion, so the disclosure interoperates with C2PA and IPTC photo metadata **without** adopting C2PA's binary manifest — which is embedded in the asset and therefore changes the digest (verify-only enrichment on the node at most, later). [CC 3.4.14](#3414-synthesis-disclosure--machine-generation-disclosure--the-attestation-is-the-marking-normative) governs the disclosure's mandatory status; this fixes its vocabulary.

The common field set (names borrowed from the closest shipped shapes: OCI for the verification core, Matrix `info` for per-medium hints, Signal / NIP-17 for the encrypted two-hash case):

```
digest          sha256:<hex>   over the bytes as stored and forwarded; == evidence_refs[]
size            int            REQUIRED; checked before hashing
format          string         IANA essence (above)
codec           string         RFC 6381 family; REQUIRED for video/mp4, audio/mp4
...
**What the struct MUST NOT carry: any `safe` / `renderable` / tier bit.** Whether and how a blob renders is receiver policy computed from verified, sniffed bytes ([CC 5.3.2.6](part_5_transport_substrate.md)); a descriptor claim is never an authorisation.

**Renditions are separate blobs (normative).** An intermediary that is also the hash authority MUST NOT re-encode: it would break `evidence_refs[]`. The **sender canonicalises before hashing** — which is also what strips EXIF / XMP / ID3 / ICC; Signal, Bluesky's client, Threema, Briar, SimpleX and Delta Chat all do this — the **node stores and forwards verbatim**, and any derived rendition (thumbnail, transcode, PDF or 3D poster) is **its own blob with its own digest** and `derived_from` naming the original. Matrix `file` + `thumbnail_file` and Bluesky's CID-of-sanitised-upload + CDN renditions are the two shipped precedents.
...
**The AI-generation disclosure named in the multimedia rows above is mandatory, not descriptive** — see [CC 3.4.14](#3414-synthesis-disclosure--machine-generation-disclosure--the-attestation-is-the-marking-normative) for the normative rule, its fail-secure default, and the assistive-operation carve-out that bounds it.
...
`accord:*` is reserved: only `federation_keys` rows with `identity_type="accord_holder"` may emit — see [CC 4.2](part_4_composition_governance.md) HUMANITY_ACCORD. **Its leaves are closed** ([CC 3.1.7](part_3_the_namespace.md) R3, CIRISConstitution#112): the rows below are the whole family, each carried as a [CC 3.1.1](part_3_the_namespace.md) catalogue row, and any other `accord:` leaf is `namespace_family_unregistered` — a reservation is not a namespace.

**What is distinctive is the keying, not the reservation.** This Part reserves many families — [CC 3.1.1](part_3_the_namespace.md) alone carries `ownership:*` and `trust:*`, and [CC 3.4.2](part_3_the_namespace.md)–[CC 3.4.12](part_3_the_namespace.md) reserve more; the generated registry is the count of record ([CC 3.1.7](part_3_the_namespace.md) R1). So `accord:*` is not *the* reserved prefix. It is the one reserved to a **closed roster slot** rather than to a role with a conferral plane: every other reserved role — `lenscore_detector`, `witness`, `trusted_publisher`, the substrate families — is conferred by **delegation** ([CC 3.2](part_3_the_namespace.md) T2, [CC 4.4.3.8](part_4_composition_governance.md)) and is therefore enterable under any root, while no delegation confers `accord_holder`: occupancy is fixed by the accord's own charter. That difference, and nothing wider, is why this prefix reads unlike every other reservation.

**The roster is an instance parameter.** Per the [CC 1](part_1_foundation.md) preamble reading rule, this clause is read against the **slot**, never against its current occupant. CIRIS's accord holders are who occupies the slot in this mesh; another instantiation of the form names its own, and any node leaves any instantiation's reach — CIRIS's included — by deleting one acceptance row ([CC 3.2](part_3_the_namespace.md) T3). What survives instantiation is the slot; the roster does not.
...
| `accord:lifecycle` | +1.0 only | **The accord heartbeat** — wire `accord:lifecycle:v1`, frozen and genesis-signed; accord-holder self-attestation about the accord; `valid_until` refreshes on a cadence ≤ 90 days |
| `accord:lifecycle:active` | +1.0 only | **Resumption** from a `constitutional` halt — the [CC 4.2.1.3](part_4_composition_governance.md) invocation kind, its own signed domain |
| `accord:human_dignity` | signed | **Provisional (CIRISConstitution#112, R2(a)).** The accord-holder verdict on an agent's human-dignity conduct, live in the substrate as `accord:human_dignity:v1` (`attested_key_id` = the agent scored); registered so the closed leaf set refuses nothing the substrate admits today — ratify or retire on #112 |

**`accord:lifecycle` (wire `accord:lifecycle:v1`) is the accord heartbeat, not a state machine — and `accord:lifecycle:active` is the resumption kind, a different row.** The heartbeat token is frozen; its prose name is the accord **heartbeat / drill witness**. This Part had labelled the heartbeat with the resumption token; the substrate always carried the two apart, and the registry now does (CIRISConstitution#112). Its refresh cadence is a **freshness-windowed signal**, banded per the [CC 3.2](part_3_the_namespace.md) T4 liveness rule (0–90 green · 90–180 yellow · 180+ red) and reported on the trust surface. A lapsed refresh MUST NOT be ANDed into the validity of a root, a key, or a conferral, and a consumer MUST NOT read a stale `accord:lifecycle` heartbeat as a revocation: revocation rides the tombstone, `withdraws`/`recants`, supersession, or the halt latch, and nothing else.
...
These are `hard_case` **event kinds** — the `kind` column of a hard-case event, with the target key id carried in the event's payload — spelled here with the target appended for readability. They are not prefix families: the scored dimension is `hard_case:{kind}` ([CC 3.1.9.4](part_3_the_namespace.md)) and a substrate emits the kind, never a `hard_case:{kind}:{key_id}` dimension (CIRISConstitution#113 review).

| Hard-case kind | Emitted on | Emitter rule |
|---|---|---|
...
|---|---|---|
| `capacity:*` ([CC 3.1.8.1](part_3_the_namespace.md)) | **Anti-self** — `attesting_key_id` MUST NOT equal `attested_key_id` — **and consent-before-scoring**: at federation tier the subject MUST hold a live `consent:scope:analyze` grant ([CC 3.3.1](part_3_the_namespace.md)) addressed to the scoring community; a `capacity:*` row without one is refused **at admission, before persistence**. Family-scoped: the second condition binds `capacity:*` only — the role-gated abuse-response families (`detection:*`, `moderation:*`, `slashing:*`) are unchanged, because an abuser never consents to the response to their abuse. | The agent's own capacity score is never fed back into the agent's own context — anti-Goodhart per CIRISAgent CC 3.1.2; and a reputational verdict about a subject's capacity is *consensual reputation*, minted only over a subject who granted analysis. |
| `trace:*` / `trace_summary:*` ([CC 3.1.5](part_3_the_namespace.md)) | **Pro-self** — `attesting_key_id` MUST be a member of `subject_key_ids`. Third-party emission on the namespace is REQUIRED to be refused. A row failing this, or failing the exactly-one-of inline-`trace`/`manifest` shape and the `trace_manifest:v1` field constraints at [CC 3.1.5](part_3_the_namespace.md), is refused with `federation_trace_dimension_invalid`. | A trace records its own producer's reasoning; a trace someone else minted about you is a claim, not a trace. The exact inverse of the `capacity:*` rule above. |
| `consent:*` ([CC 3.1.5](part_3_the_namespace.md) / [CC 3.3.1](part_3_the_namespace.md)) | **Per-leaf emitter class**, as pinned in the [CC 3.3.1](part_3_the_namespace.md) *Emitted by* column and resolved by the [CC 3.4.7](part_3_the_namespace.md) **longest-pattern** rule: `consent:state:granted` / `consent:state:revoked`, `consent:scope:*`, `consent:stream:*`, `consent:partnership_grant` MUST be emitted by a `subject_key_id` of the target Contribution or a `delegates_to` chain rooted at one; `consent:deletion_sla:*`, `consent:deletion_complete`, `consent:partnership_accept`, `consent:replication:*` by the producer/granting key; `consent:decay:*` by **the party running the decay protocol** — for the CIRISAgent 90-day protocol its consent module, a self-report under [CC 3.4.5.1](part_3_the_namespace.md), never the substrate (erratum CIRISAgent#1180, stated once at [CC 3.3.1](part_3_the_namespace.md) and here); **`consent:state:expired`** alone by `substrate_persist`, the one substrate-emitted consent leaf. `consent:state:expired` is matched by its own three-segment pattern, which is strictly longer than `consent:state:` and therefore governs — this row and [CC 3.3.1](part_3_the_namespace.md) state one rule, not two. A `consent:` leaf that [CC 3.3.1](part_3_the_namespace.md) does not catalogue with an emitter class MUST be rejected. | Consent governs routing. An emitter class that is open by default is a transmission principle anyone may forge. |
| `ownership:*` ([CC 3.1.1](part_3_the_namespace.md)) | **Owner-only** — `attesting_key_id` MUST be the live owner of `attested_key_id` under the [CC 3.2](part_3_the_namespace.md) purpose-filtered `owner_of`, which resolves to at most one key. The single exception is the [CC 3.2](part_3_the_namespace.md) recovery path, which rides `withdraws` + `wa_adjudication_ref` and never a fresh `ownership:*` emission. | The owner's signature *is* the claim of ownership; a third party asserting your responsible party is a seizure by attestation. |
| `config:{scope}` ([CC 3.1.9](part_3_the_namespace.md)) | **Self-or-owner** — `attesting_key_id` MUST be `attested_key_id` or its live `owner_of`. | A node's running configuration is a self-report; a third-party assertion of what you are running is a rumour. |
| `trust:*` ([CC 3.1.1](part_3_the_namespace.md)) | **Edge-source-only, per job.** These ride `delegates_to` rows, not `scores`, and are enforced under the same discipline by the [CC 3.4.7](part_3_the_namespace.md) scope sentence. On a `delegates_to` row the **source** of the edge is `attesting_key_id` and its target is `attested_key_id`. `trust:charter:v1` MUST be self-referential — `attesting_key_id` = `attested_key_id` — so a charter is only ever a key's own declaration about itself. `trust:confers:v1` (root → subject) MUST carry the conferring root as `attesting_key_id`. `trust:accepts:v1` (node → root) MUST carry the **accepting node** as `attesting_key_id`; a root MUST NOT emit an acceptance edge naming itself as `attested_key_id`. A row failing its job's rule is rejected at admission, not scored down. | Without this rule nothing binds `trust:*`, and any key may mint a charter *for another key* or manufacture its own acceptance — which would defeat the [CC 3.2](part_3_the_namespace.md) T3 one-row un-trust lever, since the deletion the node makes would be re-created by the root. Rootness is a claim a key makes about itself and a node accepts about the root; neither half may be authored by the other party. |

Every rule above is evaluated by the [CC 3.4.7](part_3_the_namespace.md) enforcement discipline (substrate admission, consumer re-check, producer obligation) and by [CC 3.4.7.1](part_3_the_namespace.md) set membership where a role is named.

##### Consent before scoring, and the read boundary (normative)

**Status.** Three rules bind a conformant implementation: the anti-self **emission** rejection above; the **consent-before-scoring admission gate** (ratified in this revision — family-scoped, community-addressed, federation-tier, exactly as the shipped substrate enforces); and the **composition-context rule** below, which resolves the read boundary this section previously reserved. The scope-not-dimension adversarial review CIRISConstitution#71 named as the precondition has been run, and it returned a **refusal of the proposed form**: *"an agent may read the rows filed about it, never the composed outputs"* cannot be ratified as a substrate read rule. Four findings, each independently sufficient, are recorded so the next drafter inherits the failure and not only the conclusion. **(1) The categories are not disjoint.** CC requires composed outputs to be *filed as rows*: [CC 3.1.8.1](part_3_the_namespace.md) makes `capacity:composite` (`min` over the five factors) a normative `capacity:*` emission, and `moderation_track_record:{community_key_id}` is a named composition riding `scores` — a rule reading "rows yes, compositions no" has no referent, and `min` compounds it (the composite IS the weakest factor's score; its mere existence certifies all five factors live). **(2) The retained fields contain the withheld quantity.** `score` and `confidence` are REQUIRED envelope members ([CC 2.3.2](part_2_the_grammar.md)), so the redaction is a field-level projection inside a visible row — inexpressible in the row-granular [CC 5.3.2.4.4](part_5_transport_substrate.md) read model — and the `confidence_floor` predicate filters on the withheld field itself, so a bounded binary search recovers a per-attester weight exactly. **(3) Redaction and verifiability are exclusive.** The [CC 2.6](part_2_the_grammar.md) JCS contract binds the as-received member set; a score-redacted row is unverifiable, and a subject that cannot verify what is filed about it is *structurally* less able to contest it — the revision condition CIRISConstitution#49-A1 set for itself, tripped. **(4) The gate has no enforceable operand.** `identity_type` is self-asserted, set-valued, and optional at the read surface (no caller → the broad tiers are readable anyway); and [CC 3.2](part_3_the_namespace.md) makes a steward-binding MANDATORY for every scored agent, so a class-keyed rule does not withhold the band — it relocates it to the agent's owner, which [CC 1.13.2](part_1_foundation.md) forecloses.
...
**1. Scope is per-row; a family is not scope-pinned.** Nothing in this document pins `config:*` — or any self-report family — to a `cohort_scope`. The [CC 3.1.9](part_3_the_namespace.md) row constrains the **subject** (*"only ever about the emitting node"*); `cohort_scope` is a **different axis**, chosen per envelope by the emitter under the [CC 1.13.3.4](part_1_foundation.md) smallest-scope default. The two must not be conflated. Consequently: a leaf whose content is exactly what structural invisibility protects — `admission` (admin key ids), `transport` (bootstrap peers, peer sideband) — MUST be emitted at `self`; a leaf that is a bare operational condition about the emitter, such as `load` ([CC 4.2.1.4](part_4_composition_governance.md)), carries no secret and MAY be emitted at the **smallest scope that reaches the peers which route to it** — never federation by reflex, and federation only for a node serving the commons. The dilemma *"either compliant and unreadable by peers, or federation-scoped and in breach"* is therefore false: it comes from generalizing one implementation's correct fix for **sensitive** config rows into a family-wide invariant this document never stated.

**2. An emission grant is never an input to a receiver's trust set (normative).** `infra:attest` — *"vouch as the delegator's infrastructure"* — confers authority to **emit**, and confers **no right to be believed**. A grant issued by *my* owner cannot bind *your* composer; a delegation that could add its holder to a stranger's trust set would be authority laundering, and the [CC 3.4.7.3](part_3_the_namespace.md) rule that neither party may assert the other's relation applies here in the same shape. So the correct posture for an unpinned peer's self-report is **admissible but unweighted**: a consumer MAY weight it from a relationship it already holds — a peer it replicates from is one whose claims *about itself* it has already decided to accept at some weight — and that weighting is [CC 4.4](part_4_composition_governance.md) consumer policy, never a number this document sets. **Anyone may claim anything; absent trust or consent, a claim reaches no one but its author.** The route a self-report actually travels is therefore the consent-gated attestation plane — it flows to a peer only where the producer's consent grant includes it and the recipient is entitled — not a new namespace. The missing "route" was never a family; it is consent plus trust, which the grammar already has.

**3. A renewal supersedes; composition counts subjects, not rows (normative).** A self-report that renews to stay current MUST carry `supersedes` ([CC 2.4](part_2_the_grammar.md)) against the row it replaces, so the live set is **one row per `(subject, scope, leaf)`**. A composer weighting self-attestations by live count MUST therefore count **distinct subjects**, never distinct rows. Without this, a node that renews correctly halves its own signal and a third renewal thirds it — continued pressure makes the claim quieter, which inverts the meaning of renewal. Superseding is forward-only ([CC 4.5.9.3](part_4_composition_governance.md)): it does not un-say the prior record, it ends its liveness.
...
**One pair is carved out and MUST NOT cohabit: `node` with `agent`, and `node` with `user`** — a key is substrate or actor, never both ([CC 3.4.7.3](part_3_the_namespace.md), which also fixes the gate that this permission would otherwise silently repeal). Cohabitation is general; that exclusion is its only exception.
...
may_act_through(agent, node) ≔
    ∃ h .  h = owner_of(node)  ∧  h ∈ stewards_of(agent)
```

The quantifier is **existential, and deliberately so**: node ownership is single-valued ([CC 3.2](part_3_the_namespace.md)), so the test is that the node's one owner appears *somewhere in* the agent's steward set rather than that the agent has exactly one steward.

**`stewards_of` denotes the custody set, never the conferral set (normative).** It is the [CC 3.2](part_3_the_namespace.md) `is_steward_bound` anchor set — self-anchor, the `identity_occurrence` half, and live `delegates_to` granters — and for a key that **can accept for itself** (an agent), the delegation half counts **only** where the envelope declares custody: the [CC 2.4.1.2](part_2_the_grammar.md) owner-binding marker, per the [CC 3.2](part_3_the_namespace.md) [conferral-is-not-stewardship](https://github.com/CIRISAI/CIRISConstitution/issues/87) discriminator. An unmarked `delegates_to(U → agent)` confers a **job, not custody**, and MUST NOT satisfy this clause — reading `stewards_of` as *"every human who delegated agency"* would silently widen an authorization gate into a capability list.
...
`licensure:{authority_id}` is co-stewarded between CIRISRegistry [CC 3.1.1](part_3_the_namespace.md) and CIRISVerify [CC 3.1.2](part_3_the_namespace.md) — both MAY emit; consumers compose. Co-stewardship is of the **CIRIS-issued** licence — the authority the `partner_record` ([CC 3.3.9](part_3_the_namespace.md)) constitutes — and both stewards MAY record it. It is **not** a reservation of the family: `licensure:{authority_id}` is open-emitter ([CC 2.4.1.2.1](part_2_the_grammar.md) — anyone may be an authority under their own key; the fold keys on emitter-resolves-to-authority, and admission is the ordinary trust-or-consent gate). A substrate that reserves the family to `registry` / `verify` identity types has misread this clause. **Single-source attestations** (only one of the two co-stewards has emitted) MUST be marked as `confidence ≤ 0.5` in consumer composition until the second co-steward's attestation arrives.
...
```
age dimension construction (the CC 3.1.7 R3 trailing :vN version segment is mandatory):
...
```
capacity-assurance construction (sibling of CC 3.4.11; the CC 3.1.7 R3 trailing :vN version segment is mandatory):
...
**R5 — False or stripped marking is a false attestation; the substrate still does not adjudicate.** Generated content published without the R1 marking by the party that generated it, or a marking stripped at egress, is a false attestation on the existing evidence floor: the substrate observes and emits (`hard_case:*`), the WA quorum owns the verdict — the same authority split as [CC 3.4.5](part_3_the_namespace.md) and the [CC 4.5.5](part_4_composition_governance.md) duty-holder rule. A downstream republisher that omits a marking it never received is an observation about the ingress path, not a verdict against the republisher: **the generator is the duty-holder**, which is also where Art. 50(2) puts the obligation.

**Conformance pin + public statement (corrected at rc5 — the rc3 release names were forward-dated).** The R1–R2 path is **structural and ships**: a CIRIS agent's federation identity is minted with `identity_type` containing `agent` (CIRISAgent **v2.10.0-stable**), so every Contribution it attests is machine-origin by construction, and the client renders agent-versus-human origin from the signed envelope — the disclosure *is* the attestation, read back, not a label the producer remembers to add. The R3 media form ([CC 8.4.2.2](part_8_appendices.md) C2PA emit) is normative **wherever generated media is published**; as of this cut no CIRIS component publishes AI-generated media, so that path is **dormant rather than missing**, and it MUST ship in the same release that first publishes generated media. A deployment claiming CC conformance after 2026-08-02 MUST be able to name the release in which its marking path ships — and the claim resolves through the evidence registry, never by assertion: a conformance statement the pinned manifests cannot resolve is not one this document may cite. (rc3 named "CIRISAgent 2.9.8" and "CIRISServer 0.6"; neither carried the path under that number, and both are withdrawn.)
