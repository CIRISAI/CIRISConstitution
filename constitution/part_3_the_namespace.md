...
| Prefix | Description | Polarity | Reserved? |
|---|---|---|---|
| `consent:{kind}` | Subject-side consent authority over Contributions naming the subject in `subject_key_ids`, plus the producer-side and substrate-side leaves that close its lifecycle. Leaf catalogue, per-leaf polarity and the emitter class of each leaf: [CC 3.3.1](part_3_the_namespace.md); the directed node→peer grant `consent:replication:{version}`: [CC 3.3.7](part_3_the_namespace.md). Open vocabulary per [CC 4.5.1.1](part_4_composition_governance.md). | per-leaf ([CC 3.3.1](part_3_the_namespace.md)) | **Yes — [CC 3.4.5](part_3_the_namespace.md)** |
...
|---|---|---|
| `capacity:*` ([CC 3.1.8.1](part_3_the_namespace.md)) | **Anti-self, and consent-gated** — `attesting_key_id` MUST NOT equal `attested_key_id`, **and** a live subject grant MUST cover the attester per the consent gate below. | The agent's own capacity score is never fed back into the agent's own context — anti-Goodhart per CIRISAgent CC 3.1.2. Scoring a third party is a contextual-integrity act, so the subject holds the admission key, not only a post-hoc pull. |
...
| `consent:*` ([CC 3.1.5](part_3_the_namespace.md) / [CC 3.3.1](part_3_the_namespace.md)) | **Per-leaf emitter class**, as pinned in the [CC 3.3.1](part_3_the_namespace.md) *Emitted by* column: subject-side leaves (`state`, `scope`, `stream`, `partnership_grant`) MUST be emitted by a `subject_key_id` of the target Contribution or a `delegates_to` chain rooted at one; producer-side leaves (`deletion_sla`, `deletion_complete`, `partnership_accept`, `replication`) by the producer/granting key; substrate-side leaves (`decay`, `state:expired`) by `substrate_persist`. | Consent governs routing. An emitter class that is open by default is a transmission principle anyone may forge. |
...
##### Consent before scoring — the `capacity:*` admission gate (normative)

**S1 — The gate.** A party MUST NOT emit a `capacity:*` score about a subject unless a live `consent:scope:analyze` ([CC 3.3.1](part_3_the_namespace.md)) from that subject covers the attester. This **consciously reverses**, for this one family and no other, the producer-authority default stated at [CC 2.1](part_2_the_grammar.md) (`subject_key_ids` empty = producer-only authority), [CC 2.3](part_2_the_grammar.md) / [CC 2.3.5](part_2_the_grammar.md) (post-hoc pull over *continued* processing) and [CC 3.3.7](part_3_the_namespace.md) (admission by key registration). The vocabulary is not new — `analyze` already means *derive features / scores / classifications*, is subject-emitted, positive-only and sub-scopable; what changes is that it becomes **binding on emission** rather than governing only what happens after. The reason this is a consent question and not an authority question: whether a score is *correct* is answered by recomputability and by the in-pipeline constraint diversity of [CC 3.1.5.1](part_3_the_namespace.md)–[CC 3.1.5.3](part_3_the_namespace.md) (four DMAs, six accord principles, four consciences intersecting on one reasoning chain), not by the identity of the attester set. What genuinely needs governing is *were you permitted to compute and publish this about me* — and [CC 3.3.1](part_3_the_namespace.md) already has the word for it. An emitter-standing model ("who has earned the right to score") was considered and rejected: it would make the substrate rule on worthiness, and the substrate never adjudicates.

**S2 — Consent is community-addressed, never root-addressed (MUST).** The covering grant MUST name a **bounded** set — `cohort_scope: community` + `community_id` ([CC 3.2](part_3_the_namespace.md)), founder-quorum gated and entrenched — and coverage resolves as *the attester is a live member of that community at emission time*. A grant addressed to a **trust root** MUST NOT be accepted as coverage: the trust set is unbounded and self-enrolling — every conformant deployment ships pinned to `ciris-canonical`, and [CC 3.2](part_3_the_namespace.md) already rules that the trust set and the member set are different populations — so root-addressed consent is consent to an unenumerable set, which is not consent to anything a subject could have weighed.

**S3 — The gate MUST NOT extend to the adversarial families.** `detection:*` ([CC 3.4.8](part_3_the_namespace.md)), `moderation:*` and `slashing:*` ([CC 3.1.9.2](part_3_the_namespace.md)) keep their existing duty-holder / role gates ([CC 4.5.5](part_4_composition_governance.md)) **unchanged**, and a substrate MUST NOT require subject consent to admit them. An abuser never consents to being detected; applying S1 uniformly would delete the abuse-response plane on its first use. The ratified split is exactly this: **open-sender families → consent-gated; role-gated families → unchanged.** Generalising S1 beyond the open-sender families is a conformance violation, not an improvement.

##### The anti-Goodhart read boundary (normative)

The rules above govern who may *write*. This one governs what a scored party may *read* about itself — the surface CC has so far left unruled, and where two commitments collide **exactly when the subject is an autonomous agent**: contextual integrity gives every subject access to what is filed about them, while the anti-Goodhart discipline holds that a system optimising against a score it can see is no longer being measured by it. Both survive, because they are about different surfaces. *You may not grade your own exam; you may always contest what is filed about you.*

**S4 — The rights surface is always readable (MUST).** A scored party MUST be able to see and contest **what** is filed about it: the existence of each row naming it in `subject_key_ids` ([CC 2.3](part_2_the_grammar.md)), the row's dimension family and attester, and the handles by which it may be revoked or contested — `withdraws` / `recants` under [CC 2.4.1.1](part_2_the_grammar.md) rule (2) subject revocation, and the [CC 3.3.1](part_3_the_namespace.md) consent leaves. A substrate MUST NOT redact the existence of a row from its own subject.

**S5 — The optimization surface is never readable by its subject (MUST NOT).** A substrate MUST NOT disclose to a scored party, or to any key that party controls, the **scores, weights or bands** filed about it under `capacity:*` ([CC 3.1.8.1](part_3_the_namespace.md)) — including the composed `capacity:composite` verdict and its band. This is the read-side of the anti-self rule above: an agent that cannot emit its own score but can read it holds the Goodhart channel anyway, one step longer.

**S6 — Steward and human-class identities retain full access.** A `user`-role identity ([CC 3.4.7.1](part_3_the_namespace.md)) reading scores about **itself**, and a live steward or `owner_of` ([CC 3.2](part_3_the_namespace.md)) reading scores about a key it stewards, are unaffected by S5 — the redaction is scoped to the *scored autonomous agent's own* read path, which is the only place the channel exists. A steward MUST NOT relay a redacted score back into the scored agent's context; doing so reconstitutes exactly what S5 closes.

**Revision condition.** If redaction is shown to make subjects **structurally** less able to exercise revocation than full access would — not merely costlier — the boundary is drawn in the wrong place, and S5 is what gives way, never S4.
