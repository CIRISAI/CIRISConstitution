...
- **No permanent ownerless lock (MUST).** An owner-binding SHOULD carry `delegation_valid_until` ([CC 2.4.1.2](part_2_the_grammar.md)) so it can lapse; and a key rendered ownerless by a **provably-dead** incumbent — a **WA-adjudicated liveness finding** (a lost owner key unable to sign a `withdraws`, established by sustained non-response plus out-of-band confirmation; the death/liveness predicate is pinned per deployment, so a merely-quiet live binding cannot be withdrawn as "dead") — MUST remain **re-claimable**: a Wise-Authority / recovery authority MAY withdraw such a binding (the analogue of the minor rule's "subject with revocation authority over the binding"), so a node can never be locked ownerless forever. Ownerless is **fail-secure** (no `self`-scope delivery), never permanent.
...
**The discriminator is the prefix, not an envelope field (normative — this is what makes the substrate gate a blanket rule).** Admission needs **no** `is_primary_detection` flag and **no** envelope-shape parsing. The prefix contract *is* the whole discriminator: **any** row whose `dimension` is under `detection:*` **is** a primary detector emission and MUST satisfy `lenscore_detector ∈ attesting_key.identity_type` — the substrate **rejects** it otherwise; a **cross-attestation** MUST ride the `truth_grounding:detection:*` prefix, which carries **no** `lenscore_detector` requirement (it is a score *on* the detector's verdict, not a shadowing re-emission of it). The persist admission gate is therefore a blanket reserved-prefix rule over `detection:*` — the CC 3.4 reserved-prefix `identity_type` set-membership mechanism already in `default_reserved_prefix_rules()` — with no per-row shape inspection ([CIRISPersist#365](https://github.com/CIRISAI/CIRISPersist/issues/365)).
