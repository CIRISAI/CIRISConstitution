...
**What "holds licence authority" is, on the wire (CIRISPersist#814).** Persist shipped the emitter half of the `license` delegated scope and stopped at the quorum half because no object named the authority. CC 2.4.1.2.1 now rules it: `A` is an authority iff a current steward-quorum-admitted `partner_record` with `org_id = A` exists; `X` holds it iff `X`'s key has a current `OrgAdmin` / `KeyManager` `org_membership` in `A`; the quorum is the CC 3.2 founder-quorum steward set over the `partner_record` bytes, not the accord roster or the Registry+Verify pair; refusals `licensure_authority_unknown` / `licensure_delegator_not_authority`; unknown authority fails closed. No bootstrap circle: the conferring record is `partner_record`-shaped, not `licensure`-shaped. CC 3.3.9 pins `authority_id = org_id`. The reference Registry's `/v1/partner/{key_id}` projects the legal registration number instead (`PartnerRecord.organization_id`) — filed to CIRISRegistry.
