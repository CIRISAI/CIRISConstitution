...
**Version (normative).** `version = 3` is the only current value. The mesh is seeding, so there is no legacy producer and **no deprecation window**: a verifier MUST reject a tier at `version < 3` as inadmissible — the multiplicity gate requires `version ≥ 3` and the dominance gate `version ≥ 2`, and both fail **closed** on a lower version. A flag-day cut, not a migration.
