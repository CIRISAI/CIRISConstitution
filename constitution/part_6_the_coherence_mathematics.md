...
The v3 members are **appended**, so the v2 preimage is a byte-exact prefix of the v3 preimage and the domain separator is unchanged; discrimination is carried by the `version` field alone.

**Version (normative).** `version = 3` is the only current value. The mesh is seeding, so there is no legacy producer and **no deprecation window**: a verifier MUST reject a tier at `version < 3` as inadmissible. The R9 multiplicity gate below requires `version ≥ 3` (only v3 carries `max_source_multiplicity`) and the mass-dominance gate over the signed `n_eff` requires `version ≥ 2`; both fail **closed** on a lower version. A flag-day cut, not a migration.

**R9 (content-multiplicity gate) — CEG-pinned constants (normative).** `max_source_multiplicity` and `mass_commitment` are covered by the bound-hybrid signature, so a divergence in how they are *computed* is not cosmetic: it is a fork in admissibility, with valid signatures on both sides. They are therefore **pinned normative constants, not implementation-tunable knobs** — two conformant substrates MUST NOT disagree on which folds are admissible (the same discipline as the pinned `(R, ε)` default in [CC 6.1.2](#612-the-noise-floor--unified-retirement--forever-memory-model-normative)). A conformant implementation MUST use:
...
- **`mass_commitment`** — the [CC 6.1.1](#611-wholenesswitness-divergence-detection-witness) Merkle construction over `(member_id, mass_to_fixed(mass))` leaves at the pinned `1e6` fixed-point scale, in the same lexicographic member-id order as `member_commitment`. It makes a lying `n_eff` or multiplicity **mechanically provable** from held evidence rather than merely slashable in principle.
...
// DEFAULT_MIN_VIABLE — WITHDRAWN (was 5); see the min_viable note below
DEFAULT_TARGET_HOLDERS = 30 // distinct peers holding ≥1 symbol
```

**Derivation — WITHDRAWN (informative).** The former `target_holders >= max(C_1, C_2, C_3)` derivation was arithmetically wrong and is retired. `target_holders = 30` stands as an **operating default pending re-derivation**, not as a result. Recomputed honestly:

- **The binomial model only closes at `R = N+K`.** With one *distinct* symbol per peer there are at most `N+K = 26` distinct symbols, so `target_holders = 30` cannot be the model's `R`; four peers necessarily hold duplicates and the binding count is 26.
- **The stated survival target is not met.** Reconstruction needs `X >= N = 20` for `X ~ Bin(26, q)`: at `q = 0.85` that is **0.917**, not the claimed 99.95%; `q = 0.95` gives 0.9998 and `q = 0.80` gives 0.747. Reaching 99.95% at `q = 0.85` needs `N+K >= 32` — `K >= 12` at `N = 20`, twice the shipped headroom. The "99.95% at `q = 0.85`" design target is therefore **withdrawn as unmet**, not quietly restated at a weaker figure.
- **`C_2 = 7` is struck** — it was asserted with no derivation, and the ALM fan-out figures it quoted bound viewers per copy, never holder count.
- **`C_3 = 10`** (one holder per locality, a 10-locality mission deployment) is a **declared deployment assumption**, not a derived bound, and binds nothing on its own.

**Why these and not 22 / 40 (informative).** `N=20` keeps `K=6` a meaningful ~30% FEC while one-symbol-per-peer holds across the trust island without crowding, and sits in the RaptorQ O(N²)-decode sweet spot (microsecond scale). `K=6` is RFC 6330's empirical overhead profile; the former "drops decode below 99% at q=0.85" gloss came from the withdrawn model and is struck.

**`min_viable` under all-or-nothing decode (the corrected semantics).** RaptorQ decode is all-or-nothing at block level (CC 6.1), so a layer yields its content iff at least its **own** `N` source-equivalent symbols are held, and yields **nothing** below that. `min_viable` is therefore not a tunable fraction of `N`: any value strictly below a layer's own `N` leaves the whole interval between them indistinguishable from the EnvelopeOnly tier, since nothing decodes there either. The former `DEFAULT_MIN_VIABLE = 5` was an "N/4 floor" derived under a **partial-decode** reading of RaptorQ that does not hold; against `DEFAULT_N_SOURCE = 20` it names an interval (5–19 symbols) in which residual fidelity is exactly 0. It is **withdrawn pending re-derivation** rather than replaced by a guessed constant. The graceful-degradation tier (`EjectToTier` — "still recoverable, lower fidelity") is unaffected: it is produced by **dropping whole high-detail layers** while the retained layers decode completely (CC 6.1, CC 6.1.2), never by holding a fractional symbol count. The `min_viable_symbols >= 1` floor above remains necessary but is **not** sufficient under these semantics.
...
*Drift note.* An earlier draft multiplied by an additional `(1 − ρ̄)` factor. That double-counted correlation — `k_eff` already discounts it — and drove `J = 0` at `ρ̄ → 1`, contradicting the single-constraint floor. The form above is the bare Kish identity (**identity**; `lean:coherence-ratchet:Core.BaseIdentity`) with no second correlation factor; the correction is carried by the identity itself, not by any external validation.
...
**J is monotone in diversity — and the operating corridor is withdrawn (normative citation discipline).** `J` is strictly decreasing in `ρ̄` and is maximised at `ρ̄ = 0`: it is a **throughput index**, and nothing in this section supports an intermediate "healthy band" of correlation. `J` MUST NOT be cited as the basis of an operating corridor.

**The corridor is retired, not re-founded.** Earlier drafts pinned a band `(ρ_lower, ρ_upper) = (0.1, 0.43)` for `ρ̄` — that a federation must sit strictly between total rigidity and total fragmentation. **This document states no such band, and an implementation MUST NOT gate on one.** The withdrawal is not a wording problem: the endpoints rested on an axiom that *posits* them rather than on any derivation; the rigidity arm has no measured support and the maximum measured `ρ` is `0.171` against a `0.43` edge; and the institutional `k_eff` corpus offered in support is inverted on every analyzable row. The re-basing is tracked at **RATCHET#17**; the campaign that would license a replacement claim is **pre-registered there and has not been run**, so it is named as a pending campaign and MUST NOT be cited as evidence for anything here.

**What survives, and at what strength.** Only the one-sided ceiling, as **identity**: `k_eff ≤ 1/ρ̄` is a limit of the Kish definition (`lean:coherence-ratchet:Core.BaseIdentity`), not a finding about any federation — it caps effective diversity from above and says nothing whatever about a *lower* bound on `ρ̄`. With it, the **cited-external** "scale cannot restore collapsed diversity" (CCA v5 Möbius/ceiling core, above): copies of one voice count as one voice, and no amount of echo buys more. That consequence needs no corridor, no healthy band, and no collapse asymmetry — only the ceiling.

**The gap, stated.** Two distinct quantities were previously run together here: `k_eff`, a Kish effective count over constraint orientations, and the *whole-only share*, a capacity of a collective over its parts. A claim about one MUST NOT be transferred to the other, and the retired corridor did exactly that — its whole-only-share limbs were reported at `k = 3`, while §6.2.1 requires `k ≥ 3` and the argument concerns many constraints. Symmetric noise mints even-order structure from four slots up, so the rigidity pole carries a **positive floor at `k ≥ 4`**: "both poles are exactly zero" does not hold in the regime that matters, and this document does not assert it. Whether the pole result extends past three slots at all is open at RATCHET#17. Those limbs also cited `CIRISOntology`, which is **not pinned, not vendored, and not in `claims.tsv`** — CI cannot see it — so no claim here rests on it.
