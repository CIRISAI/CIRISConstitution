...
**`org_id` is the `authority_id` (normative — ruling on CIRISPersist#814).** The `authority_id` segment of `licensure:{authority_id}` is the licensee organisation's `org_id` — the same UUID `org_membership.org_id` joins on and the same one the steward quorum admitted — **not** the legal registration number the reference Registry's `PartnerRecord.organization_id` carries (jurisdiction-scoped, mutable, PII-adjacent, and never quorum-signed on its own). A `partner_record` whose `org_id` is `A` is the object that makes `A` an authority; the full rule — what "holds licence authority for `A`" means, what the quorum is over, and why the first `licensure:{A}` row is not circular — is at [CC 2.4.1.2.1](part_2_the_grammar.md).
