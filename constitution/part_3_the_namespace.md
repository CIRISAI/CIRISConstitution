...
- **`size` (bytes) is REQUIRED on every multimedia Source struct.** It is the one field the rows above omit and the one every byte-verifying system needs. Verify size **before** computing the digest: a length mismatch is a refusal without reading the whole blob, and the descriptor alone lets a receiver enforce caps and refuse a `ContentFetch` that overruns.
