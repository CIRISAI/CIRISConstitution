...
| `session_id` | no (REQUIRED on `session:*`) | The exchange a `session:*` claim is about; with `community_id` it is the claim's key ([CC 3.1.3.1](part_3_the_namespace.md)). Verbatim, case-preserved. |
| `claimed_at` | no (REQUIRED on `session:*`) | When the claiming occurrence took the session, in the [CC 2.6.2](#2.6.2) canonical form; the convergent merge orders on it ([CC 3.1.3.1](part_3_the_namespace.md)). |
| `scope` | no (REQUIRED on `consent:state:granted`) | What the row covers: a bare token or an array of tokens. On a `consent:state:granted` row it is **the grant's scope** ([CC 3.3.1](part_3_the_namespace.md): `retain` · `share` · `analyze` · `train` · `publish`, sub-scoped in the token as `retain:90d`; a grant naming no scope matches nothing); on a `delegates_to` it is the delegated scope set ([CC 4.4.3.4.3](part_4_composition_governance.md)). Inside the signed bytes like every member; a pre-#103 consumer preserves it per [CC 2.1.1](#2.1.1) and MUST NOT compose a `capacity:*` verdict that ignores it. Listed here because every live producer already emits it (CIRISConstitution#113 review). |
...
**Why the tag is mandatory and bare-hex is rejected**: a `federation_keys.key_id` **was**, in the pre-FSD-003 reference Registry, `hex(sha256(ed25519_pubkey))` — a **lowercase 64-char hex string** — and legacy directories still hold such ids; since FSD-003 a `key_id` is the `<label>-<fingerprint>` form of [CC 2.6.8](#2.6.8), which contains `-` and base32 letters and is therefore format-distinguishable from a bare hash by construction. The tag stays mandatory because the legacy ids exist and because a subject reference MUST never depend on a consumer's guess about which form it is reading. In the original argument ([`crypto::HybridCrypto::fingerprint`](../../rust-registry/ciris-registry-core/src/crypto/mod.rs)). A bare canonical-hash (also lowercase 64-char hex) would be **format-indistinguishable** from a key_id. A "base64 key_id vs hex canonical-hash" disambiguation heuristic FAILS because Registry key_ids are themselves hex fingerprints, not base64 pubkeys. The tag is therefore load-bearing, not cosmetic. The CC 2.6.3 hex rule governs the `{hex}` segment only; the `canonical:{hashalg}:` prefix is a tagged-union discriminator outside CC 2.6.3's scope and is exempt from the "no separators" rule.
...
| triple `("discord","user","")` | `{id}` is empty; the production requires ≥ 1 scalar value |
| triple `("discord","user","1234␣")` (trailing space) | `{id}` ends in `White_Space` — reject, do NOT trim (P1) |
| triple `("discord","user","12␊34")` (embedded LF; likewise CR, TAB, NUL) | `{id}` contains a C0 control |
| triple `("matrix","user","@a\uD800:example.org")` (unpaired surrogate) | not a Unicode scalar value — `utf8(id)` is undefined; reject, never WTF-8 and never U+FFFD |
...
**A grant is non-transferable by default (normative).** A `key_grant` conveys access to an asset; it does **not** convey the power to convey it. An onward grant is a **new grant issued by the asset's owner or steward, or by a holder of a `grant`-scoped delegation from them** ([CC 4.4.3.4.3](part_4_composition_governance.md)), never a forwarded one, and `rotation_chain` is supersession lineage — not onward transfer. **Possession of the data-encryption key is not issuance authority.** Every recipient necessarily holds the DEK, having unwrapped `wrapped_dek`, so the ability to wrap a new grant is *necessary* and never *sufficient*: the cryptographic gate stops a stranger, and only the authority gate stops a recipient. A substrate MUST refuse at admission a `key_grant` whose issuer neither owns or stewards the asset nor holds a `grant`-scoped delegation over it, exactly as it refuses an issuance whose delegator lacks the underlying authority. A design that treats receipt of a grant — or possession of its key — as authority to re-grant has, again, mistaken a grant for a delegation (CIRISConstitution#113 review).
...
| `scope` | CEG 0.6 (consent; #103 erratum names the carrier) | absent | member absent | `"scope":["retain","share"]` or `"scope":"retain:90d"` |
| `session_id` | 1.0-rc5 (CC 3.1.3.1) | n/a (REQUIRED on `session:*`) | member absent (not a claim) | `"session_id":"s1"` |
| `claimed_at` | 1.0-rc5 (CC 3.1.3.1) | n/a (REQUIRED on `session:*`) | member absent (not a claim) | `"claimed_at":"2026-08-30T12:00:00.000Z"` |
...
1. **The contract bound (interop-visible).** `|JCS(envelope)| ≤ 1 MiB`. This is the only bound peers must agree on; it is measured on the bytes the signature covers and is therefore decided **after** parse. A CCP MUST NOT emit above it; a CCS MUST reject above it with `ENVELOPE_TOO_LARGE`.
...
**The two coincide on a conformant relay path.** A relay that re-emits has already recomputed JCS to verify, and [CC 2.6.1.1](#2.6.1.1) requires it to re-canonicalize member order while holding member presence/absence fixed — so what it forwards is the canonical form, and there the received octets **are** the canonical octets. The gap exists only where a non-canonical serialization first enters. Consequently **the interoperable serialization is the canonical one**: a party that transmits an inflated serialization of a semantically-conformant envelope MAY have it refused by a receiver's intake ceiling, and that refusal is conformant receiver behavior, not a receiver defect.
...
**`key_id` format (normative — FSD-003 §4).** `key_id = "<label>-<fingerprint>"`, `fingerprint` = the first **10 base32 characters** (50 bits) of `sha256(ed25519_pubkey)`, lowercased; `label` is lowercased and reduced to `[a-z0-9-]`, cosmetic. **Collision-resistant by construction, not collision-free** — the suffix is bound to the key, so two entities choosing one label collide only if their keys' hashes share 50 bits, which under one label becomes likely near **2²⁵ keys** (the birthday bound), not 2⁵⁰; **verifiable** — anyone recomputes the suffix from the pubkey (a random UUID cannot do this: one could claim another's); friendly — `eric-moore-k7f3qd2pza` reads as a name. **Registry global-uniqueness is therefore load-bearing, not a backstop**: a registrar MUST refuse a second key under an already-registered `key_id`, and a consumer resolving a `key_id` MUST verify the fingerprint against the pubkey it obtains ([CC 2.6.8](part_2_the_grammar.md) constraint above), so an aliasing collision is detected at registration and never at resolution. A deployment expecting more than **2¹⁶ identities under one label** SHOULD raise the fingerprint length (16 characters, 80 bits, moves the bound to 2⁴⁰); FSD-003 owns that change and this Part follows it (CIRISConstitution#113 review).
