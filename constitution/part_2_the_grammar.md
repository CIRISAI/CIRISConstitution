...
**The authority object, the quorum, and the bootstrap (normative — ruling on CIRISPersist#814).** "X holds licence authority for `A`" is not a `licensure:*` row, not an accord-family membership, and not a delegation. It is a conjunction of two [CC 3.3.9](part_3_the_namespace.md) operational records, both already on the wire: **(i) `A` is an authority** iff a current `partner_record` with `org_id = A` exists — not withdrawn, not superseded, `status ≠ revoked` — whose steward signature *set* over its JCS bytes verified at admission; **(ii) X holds it** iff X's key is bound by a current `org_membership` in `A` with role `OrgAdmin` or `KeyManager` (the two roles the reference Registry lets manage keys), resolved by the [CC 4.4.3.4.3.1](part_4_composition_governance.md) role-chain resolver. **The quorum is the [CC 3.2](part_3_the_namespace.md) founder-quorum steward set** — M-of-N over the `partner_record` bytes — not the accord-holder roster, not a per-authority roster, and not the Registry + Verify co-steward pair; those two are *recorders* under [CC 3.4.9](part_3_the_namespace.md), and a co-steward's own `licensure:{A}` emission is admitted on (i) alone (a real authority `A`), while a `license`-scoped issuance ([CC 4.4.3.4.3](part_4_composition_governance.md)) is admitted only when its **delegator** satisfies (i) *and* (ii). Refusals: **`licensure_authority_unknown`** (no admitted `partner_record` for `A`) and **`licensure_delegator_not_authority`** (the delegator holds no qualifying membership in `A`). **The emitter of the `partner_record` is the licensing body's Registry**, carrying the stewards' signature set; Verify admits it by the founder-quorum path. **There is no bootstrap circle**: the record that confers authority is `partner_record`-shaped, not `licensure`-shaped, so the first `licensure:{A}` row is gated on a row whose own admission is quorum — and the quorum's keys are rooted at the [CC 4.4.3.8](part_4_composition_governance.md) trust root, the one thing that is *not* in-grammar-conferred. A substrate that does not yet carry `partner_record` / `org_membership` MUST report `licensure_authority_unknown` for every `A` rather than admit "any `A`"; unknown authority fails closed.
