...
##### Consent before scoring, and the read boundary — reserved design space (non-normative)

**Status.** Two rules bind a conformant implementation: the anti-self rejection above, **and the consent-before-scoring admission gate — ratified in this revision**. The gate is **family-scoped** to `capacity:*` and **community-addressed** — a grant is addressed to the scoring community, never the root, because root-addressed consent is consent to an unenumerable, self-enrolling set (the [CC 3.2](part_3_the_namespace.md) trust model's own reasoning) — and it runs at **federation-tier admission, before persistence**. The shipped substrate enforces exactly this rule (CIRISPersist `check_capacity_consent_admission`, pinned to the `consent:scope:analyze` wire token, three-backend differential-tested; gating production emission on the canonical since 2026-08-01) — ratification here makes the spec and the mesh agree rather than declaring the mesh non-conformant. The adversarial findings recorded at CIRISConstitution#46 — capture via owner-sibling keys, third-party-dissent removal, the duty-holder exemption boundary — are the standing analysis this rule answers; the abuse-response carve-out on the emitter row is the exemption boundary's resolution. **The read boundary remains reserved design space** (CIRISConstitution#49): no read rule redacts `capacity:*` from any party the [CC 5.3.2.4.4](part_5_transport_substrate.md) scope gate admits. The shipped read surface returns a subject its own composed verdict and band, and ratifying a never-the-band rule would declare *that* surface non-conformant without an adversarial review of the scope-not-dimension read model — a deployment MUST NOT enforce a read boundary ahead of that ratification and call itself conformant.
