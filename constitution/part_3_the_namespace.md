...
|---|---|---|
| `capacity:*` ([CC 3.1.8.1](part_3_the_namespace.md)) | **Anti-self** — `attesting_key_id` MUST NOT equal `attested_key_id`. (The consent-before-scoring gate described below is **deferred and non-normative**; it is not a second condition on this row.) | The agent's own capacity score is never fed back into the agent's own context — anti-Goodhart per CIRISAgent CC 3.1.2. |
...
##### Consent before scoring, and the read boundary — DEFERRED, non-normative

**Status.** Two proposals were drafted against this section — a consent-before-scoring admission gate on `capacity:*` (S1–S3) and an anti-Goodhart read boundary (S4–S6). Both are **deferred and carry no normative force in this revision**: nothing below is a conformance condition, no implementation is non-conformant for omitting it, and the `capacity:*` rule that binds is the anti-self rule in the table above and nothing else. They are recorded rather than deleted because the questions they raise are real and the record of *why the drafted answers failed* is what a future attempt needs.

**What was proposed.** *S1–S3* would have made a live `consent:scope:analyze` from the subject, covering the attester, a precondition on **emitting** a `capacity:*` score rather than a control over continued processing — with coverage addressed to a bounded community rather than to a trust root (S2), and with the adversarial families `detection:*` / `moderation:*` / `slashing:*` explicitly exempted, since an abuser never consents to being detected (S3). *S4–S6* would have split the read surface: the *rights* surface — that a row exists, its family, its attester, and the handles to revoke or contest it — always readable by its subject (S4), while the *optimization* surface — the `capacity:*` scores, weights and bands themselves — was withheld from the scored agent and from keys it controls (S5), with stewards and human-class identities unaffected (S6).

**Why they are deferred.** Four findings, any one of which is disqualifying:

- **The gate is capturable by the party it constrains.** An owner can mint a sibling key and score itself through a community it controls. The anti-self rule tests *key equality* while the coverage test asks about *membership*, so a captive community satisfies the consent test that key equality would have failed — the gate admits exactly the self-scoring it was added to prevent.
- **It removes third-party dissent from the one family designed to be third-party-only.** `capacity:*` is anti-self precisely so that the record about an agent is authored by others; making that authorship contingent on the subject's grant lets a subject suppress unflattering scores by declining to consent, which is a stronger power than the post-hoc pull the design deliberately settled on.
- **The S3 exemption boundary rests on a role gate that does not exist.** The split assumes the adversarial families are role-gated where the open-sender families are not; `slashing:*` demonstrably lacks such a gate, so the boundary cannot be drawn where S3 draws it.
- **S5 is unimplementable against the shipped read model.** [CC 5.3.2.4.4](part_5_transport_substrate.md) states that reads are gated by **scope**, not by dimension authority. A per-dimension redaction on the read path is not expressible in the query surface CC actually specifies, and specifying it would mean re-opening the closed-operator discipline that read surface exists to hold.
