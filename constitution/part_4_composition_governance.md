...
**The swap test — a mandatory drafting gate (normative).** Every clause that **confers authority or gates a capability** MUST pass the swap test before adoption: *swap the most and least powerful parties the clause names or implies, re-read it, and ask whether it still reads acceptable; if it does not, it was a privilege, not a rule.* This is a drafting gate on what CC may say, never an adjudication rule — it binds amendment and the founder's pre-maturity authority alike. The outcome is recorded with the clause: on its `claims.tsv` evidence line where it carries one, and — so that silence means something — **exhaustively** in the declared-asymmetry register below where the test fails. A clause absent from that register asserts that it passed.
...
| **Genesis root-minting** — the accord triple can confer the first `canonical` when no prior root exists ([CC 4.2.1](#421-authority--authority-scope)) | the existence of a second root: anyone may mint one by the same ceremony, and the privilege is first-mover, not structural | un-trust and re-root ([CC 4.4.3.8](#4438-policy-direct--policy-a--direct-trust)); the base self-root keeps the installation running meanwhile | conferral only — accord authority cannot reach the consent, licensure, or amendment planes |

The [CC 4.2](#42-accord--the-humanity_accord-constitutional-layer) halt itself is **not** on this register: it passes the swap test on the [CC 4.2.5](#425-isn--why-this-isnt-a-golden-rule-violation) reading, because the party bound is the party who granted and can ungrant. **Revision condition:** if the register grows past a handful of entries, or if an entry's stated bound passes without the asymmetry ending, the form has failed as a discipline and the surviving asymmetries should be re-read as permanent structure requiring [CC 4.5.1.2](#4512-meta-amendment--entrenchment) entrenched ratification rather than declaration.
...
**The fifteen proposed classifications — twelve ratified, two ratified, one revised.**

**`n/a` — ratified for all twelve** (`achieved_tier`, `committed_tier`, `asserted_at`, `validity_window`, `community_key_id`, `confidence`, `context`, `enabled`, `evidence_refs`, `scope`, `tenant_id`, `withdrawal_reason`), with `n/a` read strictly as *no processor fuses these into a decision*. That reading is falsifiable, and two members carry a live falsifier: **`scope`**, because `consent:scope:{retain|share|analyze|train|publish}` is a permitted-use gate ([CC 4.5.2.2](#4522-compliance-vertical--vertical-compliance-mapping-informational)), and **`enabled`**, because a capability toggle is a decision by construction. If a processor is found gating across axes on either, `n/a` is wrong and the correct value is `logical_defect` with a tracking ref.

**`axiomatic_intent` — `withdraws` ratified.** A `withdraws` *is* both a `recipient_revoke` action and a `temporal_lifecycle` transition: one act, two axes, by definition. Its authority arc is guarded on the separate subject/attester axis, not on this field.

**`axiomatic_intent` — `consent` ratified, with the direction constraint pinned.** Consent is definitionally a `transmission_principle` that gates `recipient_receive` and carries a `temporal_lifecycle`. What MUST NOT ride the same field is **direction**: send-versus-receive is a distinct discriminator, and fusing it here reproduces downstream precisely the overload it was split out to avoid.

**`axiomatic_intent` — `references_attestation_id` revised, not ratified as proposed.** `withdraws` is *one operation spanning two axes*; `references_attestation_id` is *one slot carrying three different relations across three operations* (revoke-target, temporal-prior, subject-binding). That is polysemy — the shape this gate was built to catch — not an action that spans axes by definition, and it is the same pointer-decides-authority shape as the tracked defects. Splitting the wire field would cost envelope surface the [CC 1.7](part_1_foundation.md) 1+4 freeze does not permit, so the split falls on the **classification**, not the field:
